...
namespace Opsive.BehaviorDesigner.Editor.Controls.NodeViews
{
    ...
    public class UtilityEvaluatorNodeViewControl : TaskNodeViewControl
    {
        ...
        private void UpdateUtilityValue()
        {
            ...
            UtilityEvaluator utilityEvaluator = m_BehaviorTree.GetTask(taskObjectComponents[m_UtilityEvaluatorComponentIndex].Index) as UtilityEvaluator;
            if (utilityEvaluator == null)
            {
                return;
            }

            m_UtilityValueLabel.text = "Value: " + Math.Round(utilityEvaluator.GetUtilityValue(), 3);
        }
    }
}
#endif