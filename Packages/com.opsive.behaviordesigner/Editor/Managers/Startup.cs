...
namespace Opsive.BehaviorDesigner.Editor.Managers
{
    ...
    public class Startup
    {
        ...
        private static void EditorStartup()
        {
            ...
            string[] importStatusAssets = AssetDatabase.FindAssets("t:ImportStatus");
            if (importStatusAssets != null && importStatusAssets.Length > 0)
            {
                for (int i = 0; i < importStatusAssets.Length; ++i)
                {
                    string path = AssetDatabase.GUIDToAssetPath(importStatusAssets[i]);
            ...
            {
                MainManagerWindow window = BehaviorMainWindow.ShowWindow();
