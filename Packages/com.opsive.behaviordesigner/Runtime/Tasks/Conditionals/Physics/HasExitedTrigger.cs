...
{
    [Description(
        "Returns success when an object exits the trigger. This task will only receive the physics callback if it is being reevaluated (with a conditional abort or under a parallel task).")]
    [Category("Physics")]
    public class HasExitedTrigger : Conditional
    {
        ...
        protected override bool ReceiveTriggerExitCallback { get => true; }
