...
namespace Opsive.BehaviorDesigner.Runtime.Tasks.Composites
{
    ...
    [Description("Similar to the sequence task, the parallel task will run each child task until a child task returns failure. " +
    ...
    public partial struct ParallelTaskSystem : ISystem
    {
        ...
        private partial struct ParallelJob : IJobEntity
        {
            ...
            [BurstCompile]
            public void Execute(Entity entity, [EntityIndexInQuery] int entityIndex, ref DynamicBuffer<ParallelComponent> parallelComponents, ref DynamicBuffer<TaskComponent> taskComponents,
                ref DynamicBuffer<BranchComponent> branchComponents)
            {
                for (int i = 0; i < parallelComponents.Length; ++i)
                {
                    ParallelComponent parallelComponent = parallelComponents[i];
                    TaskComponent taskComponent = taskComponents[parallelComponent.Index];
