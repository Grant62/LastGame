...
namespace Opsive.BehaviorDesigner.Runtime.Tasks.Composites
{
    ...
    [Description("Similar to the selector task, the random selector task will return success as soon as a child task returns success.  " +
    ...
    public class RandomSelector : ECSCompositeTask<RandomSelectorTaskSystem, RandomSelectorComponent>, IParentNode, IConditionalAbortParent, IInterruptResponder, ISavableTask, ICloneable
    {
        ...
        {
            DynamicBuffer<RandomSelectorComponent> randomSelectorComponents = world.EntityManager.GetBuffer<RandomSelectorComponent>(entity);
            RandomSelectorComponent randomSelectorComponent = randomSelectorComponents[m_ComponentIndex];
            ...
            {
                ushort[] taskOrder = randomSelectorComponent.TaskOrder.Value.Indicies.ToArray();
        ...
        public object Clone()
        {
            RandomSelector clone = Activator.CreateInstance<RandomSelector>();
            clone.Index = Index;
    ...
    public partial struct RandomSelectorTaskSystem : ISystem
    {
        ...
        {
            foreach ((DynamicBuffer<BranchComponent> branchComponents, DynamicBuffer<TaskComponent> taskComponents, DynamicBuffer<RandomSelectorComponent> randomSelectorComponents,
                         Entity entity) in
                     SystemAPI.Query<DynamicBuffer<BranchComponent>, DynamicBuffer<TaskComponent>, DynamicBuffer<RandomSelectorComponent>>().WithAll<RandomSelectorFlag, EvaluateFlag>()
                         .WithEntityAccess())
            {
                for (int i = 0; i < randomSelectorComponents.Length; ++i)
                {
                    RandomSelectorComponent randomSelectorComponent = randomSelectorComponents[i];
                    TaskComponent taskComponent = taskComponents[randomSelectorComponent.Index];
                    ...
                    DynamicBuffer<RandomSelectorComponent> randomSelectorComponentsBuffer = randomSelectorComponents;
                    ...
                    if (taskComponent.Status == TaskStatus.Queued)
                    {
                        ...
                        {
                            randomSelectorComponent.RandomNumberGenerator = Random.CreateFromIndex(randomSelectorComponent.Seed != 0 ? randomSelectorComponent.Seed : (uint)entity.Index);
                        }

                        // Use fisher-yates to shuffle the array in place.
                        ref BlobArray<ushort> initialTaskOrder = ref randomSelectorComponent.TaskOrder.Value.Indicies;
                        int index = initialTaskOrder.Length;
                        while (index != 0)
                        {
                            float randomUnitFloat = randomSelectorComponent.RandomNumberGenerator.NextFloat();
                    ...
                    ref BlobArray<ushort> taskOrder = ref randomSelectorComponent.TaskOrder.Value.Indicies;
                    TaskComponent childTaskComponent = taskComponents[taskOrder[randomSelectorComponent.ActiveRelativeChildIndex]];
                    ...
                    else
                    {
                        ...
                        ushort nextIndex = taskOrder[randomSelectorComponent.ActiveRelativeChildIndex];
        ...
        {
            foreach (DynamicBuffer<RandomSelectorComponent> randomSelectorComponents in SystemAPI.Query<DynamicBuffer<RandomSelectorComponent>>())
            {
                for (int i = 0; i < randomSelectorComponents.Length; ++i)
                {
                    RandomSelectorComponent randomSelectorComponent = randomSelectorComponents[i];
    ...
    public partial struct RandomSelectorInterruptSystem : ISystem
    {
        ...
        private void OnUpdate(ref SystemState state)
        {
            foreach ((DynamicBuffer<TaskComponent> taskComponents, DynamicBuffer<RandomSelectorComponent> randomSelectorComponents) in
                     SystemAPI.Query<DynamicBuffer<TaskComponent>, DynamicBuffer<RandomSelectorComponent>>().WithAll<InterruptFlag>())
            {
                for (int i = 0; i < randomSelectorComponents.Length; ++i)
                {
                    ...
                    TaskComponent taskComponent = taskComponents[randomSelectorComponent.Index];
                    if (taskComponent.Status == TaskStatus.Running && taskComponents[randomSelectorComponent.TaskOrder.Value.Indicies[randomSelectorComponent.ActiveRelativeChildIndex]].Status
                        != TaskStatus.Running)
                    {
                        ...
                        DynamicBuffer<RandomSelectorComponent> randomSelectorBuffer = randomSelectorComponents;
