...
namespace Opsive.BehaviorDesigner.Runtime.Tasks.Composites
{
    ...
    [Description("The selector task is similar to an \"or\" operation. It will return success as soon as one of its child tasks return success. " +
    ...
    public class Selector : ECSCompositeTask<SelectorTaskSystem, SelectorComponent>, IParentNode, IConditionalAbortParent, IInterruptResponder, ISavableTask, ICloneable
    {
        ...
        {
            DynamicBuffer<SelectorComponent> selectorComponents = world.EntityManager.GetBuffer<SelectorComponent>(entity);
            SelectorComponent selectorComponent = selectorComponents[m_ComponentIndex];
        ...
        public object Clone()
        {
            Selector clone = Activator.CreateInstance<Selector>();
            clone.Index = Index;
    ...
    public partial struct SelectorTaskSystem : ISystem
    {
        ...
        {
            EntityQuery query = SystemAPI.QueryBuilder().WithAllRW<BranchComponent>().WithAllRW<TaskComponent>().WithAllRW<SelectorComponent>().WithAll<SelectorFlag, EvaluateFlag>().Build();
        ...
        private partial struct SelectorJob : IJobEntity
        {
            ...
            public void Execute(ref DynamicBuffer<BranchComponent> branchComponents, ref DynamicBuffer<TaskComponent> taskComponents, ref DynamicBuffer<SelectorComponent> selectorComponents)
            {
                ...
                {
                    SelectorComponent selectorComponent = selectorComponents[i];
                    TaskComponent taskComponent = taskComponents[selectorComponent.Index];
                    ...
                    TaskComponent childTaskComponent = taskComponents[selectorComponent.ActiveChildIndex];
    ...
    public partial struct SelectorInterruptSystem : ISystem
    {
        ...
        private void OnUpdate(ref SystemState state)
        {
            foreach ((DynamicBuffer<TaskComponent> taskComponents, DynamicBuffer<SelectorComponent> selectorComponents) in
                     SystemAPI.Query<DynamicBuffer<TaskComponent>, DynamicBuffer<SelectorComponent>>().WithAll<InterruptFlag>())
            {
                for (int i = 0; i < selectorComponents.Length; ++i)
                {
                    ...
                    {
                        ushort childIndex = (ushort)(selectorComponent.Index + 1);
                        ...
                        DynamicBuffer<SelectorComponent> selectorBuffer = selectorComponents;
