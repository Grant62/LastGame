...
{
    /// <summary>
    ///     The TaskDelegateBase task is an abstract class used for any action classes that use reflection to execute the
    ///     action.
    /// </summary>
    [HideInFilterWindow]
    public abstract class TaskDelegateBase : Task, IAction
    {
        ...
        public SharedVariable Target { get => m_Target; }

        public string ReflectedType { get => m_ReflectedType; }

        public string[] ParameterTypes { get => m_ParameterTypes; }

        public string MethodName { get => m_MethodName; }
        ...
        protected object GetTargetValue()
        {
            ...
            object target = m_Target.GetValue();
            if (target == null)
            {
                string targetType = m_ReflectedType.Replace("UnityEngine.", string.Empty);
                if (string.Equals(targetType, "GameObject"))
                {
                    return m_GameObject;
                }

                Component value = m_GameObject.GetComponent(targetType);
                if (value == null)
                {
                    string[] splitType = m_ReflectedType.Split(".");
        ...
        {
            Type type = TypeUtility.GetType(reflectedType);
            if (type == null)
            {
            ...
            MethodInfo method = type.GetMethod(methodName, BindingFlags.Public | BindingFlags.Instance | BindingFlags.NonPublic | BindingFlags.Static, null, parameterTypes, null);
            if (method == null)
            {
    ...
    public class TaskDelegate : TaskDelegateBase
    {
        ...
        {
            MethodInfo method = GetMethod(m_ReflectedType, m_MethodName, m_ParameterTypes);
            if (method == null)
            {
                return;
            }
    ...
    public class TaskValueDelegate<T1, T2, T3, T4, T5, T6, T7, T8, T9, TResult> : TaskDelegateBase
    {
        ...
        {
            m_Result.Value = m_Delegate(m_Parameter1.Value, m_Parameter2.Value, m_Parameter3.Value, m_Parameter4.Value, m_Parameter5.Value, m_Parameter6.Value, m_Parameter7.Value,
                m_Parameter8.Value, m_Parameter9.Value);
            if (m_ConditionalTask)
            {
                return Convert.ToBoolean(m_Result.Value) ? TaskStatus.Success : TaskStatus.Failure;
            }
    ...
    public class TaskDelegate<T1, T2, T3, T4, T5, T6, T7, T8, T9, T10> : TaskDelegateBase
    {
        ...
        {
            m_Delegate(m_Parameter1.Value, m_Parameter2.Value, m_Parameter3.Value, m_Parameter4.Value, m_Parameter5.Value, m_Parameter6.Value, m_Parameter7.Value, m_Parameter8.Value,
                m_Parameter9.Value, m_Parameter10.Value);
            return TaskStatus.Success;
        }
    }
    ...
    public class TaskValueDelegate<T1, T2, T3, T4, T5, T6, T7, T8, T9, T10, TResult> : TaskDelegateBase
    {
        ...
        {
            m_Result.Value = m_Delegate(m_Parameter1.Value, m_Parameter2.Value, m_Parameter3.Value, m_Parameter4.Value, m_Parameter5.Value, m_Parameter6.Value, m_Parameter7.Value,
                m_Parameter8.Value, m_Parameter9.Value, m_Parameter10.Value);
            if (m_ConditionalTask)
            {
                return Convert.ToBoolean(m_Result.Value) ? TaskStatus.Success : TaskStatus.Failure;
            }
