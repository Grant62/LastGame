...
namespace Opsive.BehaviorDesigner.Runtime.Tasks.Actions
{
    ...
    /// </summary>
    [Description("The return status task will immediately return sucess or failure.")]
    public class ReturnStatus : ECSActionTask<ReturnStatusTaskSystem, ReturnStatusComponent>, ICloneable
    {
        ...
        public object Clone()
        {
            ReturnStatus clone = Activator.CreateInstance<ReturnStatus>();
            clone.Index = Index;
    ...
    public partial struct ReturnStatusTaskSystem : ISystem
    {
        ...
        {
            EntityQuery query = SystemAPI.QueryBuilder().WithAllRW<TaskComponent>().WithAllRW<ReturnStatusComponent>().WithAll<ReturnStatusFlag, EvaluateFlag>().Build();
        ...
        private partial struct ReturnStatusJob : IJobEntity
        {
            ...
            public void Execute(ref DynamicBuffer<TaskComponent> taskComponents, ref DynamicBuffer<ReturnStatusComponent> returnStatusComponents)
            {
                ...
                {
                    ReturnStatusComponent returnStatusComponent = returnStatusComponents[i];
                    TaskComponent taskComponent = taskComponents[returnStatusComponent.Index];
