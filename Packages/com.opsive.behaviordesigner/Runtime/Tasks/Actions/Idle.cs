...
namespace Opsive.BehaviorDesigner.Runtime.Tasks.Actions
{
    ...
    [Description("Returns a TaskStatus of running. The task will only stop when interrupted or a conditional abort is triggered.")]
    ...
    public partial struct IdleTaskSystem : ISystem
    {
        ...
        {
            EntityQuery query = SystemAPI.QueryBuilder().WithAllRW<TaskComponent>().WithAll<IdleComponent, IdleFlag, EvaluateFlag>().Build();
        ...
        private partial struct IdleJob : IJobEntity
        {
            ...
            public void Execute(ref DynamicBuffer<TaskComponent> taskComponents, ref DynamicBuffer<IdleComponent> idleComponents)
            {
                ...
                {
                    IdleComponent idleComponent = idleComponents[i];
                    TaskComponent taskComponent = taskComponents[idleComponent.Index];
