...
namespace Opsive.BehaviorDesigner.Runtime.Tasks.Actions
{
    ...
    [Description(
        "Performs the actual interruption. This will immediately stop the specified tasks from running and will return success or failure depending on the value of interrupt success.")]
    public class PerformInterruption : ECSActionTask<PerformInterruptionTaskSystem, PerformInterruptionComponent>
    {
        ...
        [SerializeField]
        private ILogicNode[] m_InterruptTasks;
        [Tooltip("Should the interrupted task return success?")]
        [SerializeField]
        private bool m_InterruptSuccess;
        ...
        public override PerformInterruptionComponent GetBufferElement()
        {
            ...
            {
                Debug.LogError("Error: At least one interrupt task must be specified.");
                return new PerformInterruptionComponent();
            }

            ushort[] indicies = new ushort[m_InterruptTasks.Length];
            int nullTaskCount = 0;
            ...
            if (nullTaskCount > 0)
            {
                Array.Resize(ref indicies, indicies.Length - nullTaskCount);
            }

            BlobBuilder builder = new(Allocator.Temp);
            ref IndiciesBlob root = ref builder.ConstructRoot<IndiciesBlob>();
            BlobBuilderArray<ushort> indicesArray = builder.Allocate(ref root.Indicies, indicies.Length);
            for (int i = 0; i < indicies.Length; i++)
            {
                indicesArray[i] = indicies[i];
            }

            BlobAssetReference<IndiciesBlob> blobAsset = builder.CreateBlobAssetReference<IndiciesBlob>(Allocator.Persistent);
            builder.Dispose();
    ...
    public partial struct PerformInterruptionTaskSystem : ISystem
    {
        ...
        {
            foreach ((DynamicBuffer<BranchComponent> branchComponents, DynamicBuffer<TaskComponent> taskComponents, DynamicBuffer<PerformInterruptionComponent> performInterruptionComponents,
                         Entity entity) in
                     SystemAPI.Query<DynamicBuffer<BranchComponent>, DynamicBuffer<TaskComponent>, DynamicBuffer<PerformInterruptionComponent>>()
                         .WithAll<PerformInterruptionFlag, EvaluateFlag>().WithEntityAccess())
            {
                for (int i = 0; i < performInterruptionComponents.Length; ++i)
                {
                    PerformInterruptionComponent performInterruptionComponent = performInterruptionComponents[i];
                    TaskComponent taskComponent = taskComponents[performInterruptionComponent.Index];
                    ...
                    if (taskComponent.Status == TaskStatus.Queued)
                    {
                        ...
                        {
                            TaskComponent interruptTaskComponent = taskComponents[performInterruptionComponent.InterruptIndicies.Value.Indicies[j]];
                            BranchComponent interruptBranchComponent = branchComponents[interruptTaskComponent.BranchIndex];
        ...
        {
            foreach (DynamicBuffer<PerformInterruptionComponent> performInterruptionComponents in SystemAPI.Query<DynamicBuffer<PerformInterruptionComponent>>())
            {
                for (int i = 0; i < performInterruptionComponents.Length; ++i)
                {
                    PerformInterruptionComponent performInterruptionComponent = performInterruptionComponents[i];
