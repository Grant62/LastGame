...
namespace Opsive.BehaviorDesigner.Runtime.Tasks.Actions
{
    ...
    [Description("Wait a specified amount of time. The task will return running until the task is done waiting. It will return success after the wait time has elapsed.")]
    public class Wait : ECSActionTask<WaitTaskSystem, WaitComponent>, ICloneable, IPausableTask, ISavableTask
    {
        ...
        [SerializeField]
        private RangeFloat m_RandomDurationRange;
        ...
        {
            m_Duration = m_RandomDurationRange.Min = m_RandomDurationRange.Max = 1;
        ...
        {
            DynamicBuffer<WaitComponent> waitComponents = world.EntityManager.GetBuffer<WaitComponent>(entity);
            WaitComponent waitComponent = waitComponents[m_ComponentIndex];
            waitComponent.PauseTime = Time.time;
            DynamicBuffer<WaitComponent> waitComponentBuffer = waitComponents;
        ...
        public object Clone()
        {
            Wait clone = Activator.CreateInstance<Wait>();
            clone.Index = Index;
    ...
    public partial struct WaitTaskSystem : ISystem
    {
        ...
        {
            EntityQuery query = SystemAPI.QueryBuilder().WithAllRW<TaskComponent>().WithAllRW<WaitComponent>().WithAll<WaitFlag, EvaluateFlag>().Build();
        ...
        private partial struct WaitJob : IJobEntity
        {
            ...
            public void Execute(Entity entity, ref DynamicBuffer<TaskComponent> taskComponents, ref DynamicBuffer<WaitComponent> waitComponents)
            {
                ...
                {
                    WaitComponent waitComponent = waitComponents[i];
                    TaskComponent taskComponent = taskComponents[waitComponent.Index];
                    if (taskComponent.Status == TaskStatus.Queued)
                    {
                        ...
                        if (waitComponent.RandomDuration)
                        {
                            ...
                            {
                                waitComponent.RandomNumberGenerator = Random.CreateFromIndex(waitComponent.Seed != 0 ? waitComponent.Seed : (uint)entity.Index);
                            }
    ...
    [NodeIcon("b4b59e888607422409f1efa599af34ae", "e1cb9cb566a90fb4489bf31465b99747")]
    [Description(
        "Wait a specified amount of time. The task will return running until the task is done waiting. It will return success after the wait time has elapsed. Uses the GameObject workflow.")]
    public class SharedWait : Action
    {
        [Tooltip("The amount of time to wait (in seconds).")]
        [SerializeField]
        private SharedVariable<float> m_Duration = 1;
        [Tooltip("The seed of the random number generator. Set to 0 to disable.")]
        [SerializeField]
        private int m_Seed;
        [Tooltip("Should the wait duration be randomized?")]
        [SerializeField]
        private SharedVariable<bool> m_RandomDuration;
        [Tooltip("The minimum wait duration if random wait is enabled.")]
        [SerializeField]
        private SharedVariable<RangeFloat> m_RandomDurationRange = new RangeFloat(1, 1);
        [Tooltip("The maximum wait duration if random wait is enabled.")]
        [SerializeField]
        private SharedVariable<float> m_RandomDurationMax = 1;
