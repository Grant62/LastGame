...
namespace Opsive.BehaviorDesigner.Runtime.Tasks
{
    ...
    public abstract class StackedTask : Task, ITreeLogicNode, IStackedNode
    {
        ...
        public ushort ActiveIndex { get; private set; }
        ...
        public void Add(object obj)
        {
            ...
            if (obj is MethodInfo)
            {
                ...
                Type[] types = new Type[(parameters != null ? parameters.Length : 0) + (methodInfo.ReturnType != typeof(void) ? 1 : 0)];
                if (parameters != null)
                {
                ...
                if (methodInfo.ReturnType == typeof(void))
                {
                    if (parameters != null && parameters.Length > 0)
                    {
                        ...
                        {
                            Debug.LogError($"Error: Unable to create TaskDelegate with {parameters.Length}. Please send this error to support@opsive.com.");
                            return;
                        }
                    }
                    else
                ...
                else
                {
                    ...
                    if (parameters != null && parameters.Length > 0)
                    {
                        ...
                        {
                            Debug.LogError($"Error: Unable to create TaskValueDelegate with {parameters.Length}. Please send this error to support@opsive.com.");
                            return;
                        }
                    }
                    else
                ...
                TaskDelegateBase actionDelegate = Activator.CreateInstance(actionDelegateType) as TaskDelegateBase;
        ...
        public override TaskStatus OnUpdate()
        {
            ...
            while (ActiveIndex < m_Tasks.Length)
            {
                if (m_Tasks[ActiveIndex] == null)
                {
                    continue;
                }

                // Call start when the local task is started, not when the StackedTask starts.
                if (!m_TaskStarted[ActiveIndex])
                {
                    m_Tasks[ActiveIndex].OnStart();
                    m_TaskStarted[ActiveIndex] = true;
                }

                TaskStatus executionStatus = m_Tasks[ActiveIndex].OnUpdate();
                ...
                if (m_ComparisonType == ComparisonType.Selector && executionStatus == TaskStatus.Success)
                {
                    return TaskStatus.Success;
                }

                if (!m_TaskEnded[ActiveIndex])
                {
                    m_Tasks[ActiveIndex].OnEnd();
                    m_TaskEnded[ActiveIndex] = true;
                }
        ...
        public override object Save(World world, Entity entity)
        {
            ...
            object[] saveData = new object[m_Tasks.Length + 1];
            for (int i = 0; i < m_Tasks.Length; ++i)
            {
                if (m_Tasks[i] == null)
                {
                ...
                MemberVisibility reflectionType = m_Tasks[i].GetSaveReflectionType(i);
                if (reflectionType != MemberVisibility.None)
                {
                    saveData[i] = Serialization.Serialize(m_Tasks[i], reflectionType, ValidateSerializedObject);
                }
                else
                {
            ...
            saveData[m_Tasks.Length] = ActiveIndex;
            ...
            ref ResizableArray<TaskAssignment> taskReferences)
        {
            ...
            object[] taskData = (object[])saveData;
            ...
            ActiveIndex = (ushort)taskData[m_Tasks.Length];
        }
        ...
        {
            MemberVisibility reflectionType = task.GetSaveReflectionType(index);
            if (reflectionType != MemberVisibility.None)
            {
                ResizableArray<TaskAssignment> localTaskReferences = taskReferences;
                (saveData as Serialization).DeserializeFields(task, reflectionType, ValidateDeserializedTypeObject,
                    (fieldInfoObj, task, value) => { return ValidateDeserializedObject(fieldInfoObj, task, value, ref variableByNameMap, ref localTaskReferences); });
