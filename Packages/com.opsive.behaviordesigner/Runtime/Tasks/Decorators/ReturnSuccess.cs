...
namespace Opsive.BehaviorDesigner.Runtime.Tasks.Decorators
{
    ...
    [Description("The return success task will always return success except when the child task is running.")]
    ...
    public partial struct ReturnSuccessTaskSystem : ISystem
    {
        ...
        {
            EntityQuery query = SystemAPI.QueryBuilder().WithAllRW<BranchComponent>().WithAllRW<TaskComponent>().WithAllRW<ReturnSuccessComponent>().WithAll<ReturnSuccessFlag, EvaluateFlag>()
                .Build();
        ...
        private partial struct ReturnSuccessJob : IJobEntity
        {
            ...
            public void Execute(ref DynamicBuffer<BranchComponent> branchComponents, ref DynamicBuffer<TaskComponent> taskComponents,
                ref DynamicBuffer<ReturnSuccessComponent> returnSuccessComponents)
            {
                for (int i = 0; i < returnSuccessComponents.Length; ++i)
                {
                    ReturnSuccessComponent returnSuccessComponent = returnSuccessComponents[i];
                    TaskComponent taskComponent = taskComponents[returnSuccessComponent.Index];
