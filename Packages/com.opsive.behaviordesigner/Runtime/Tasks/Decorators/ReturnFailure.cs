...
namespace Opsive.BehaviorDesigner.Runtime.Tasks.Decorators
{
    ...
    [Description("The return failure task will always return failure except when the child task is running.")]
    ...
    public partial struct ReturnFailureTaskSystem : ISystem
    {
        ...
        {
            EntityQuery query = SystemAPI.QueryBuilder().WithAllRW<BranchComponent>().WithAllRW<TaskComponent>().WithAllRW<ReturnFailureComponent>().WithAll<ReturnFailureFlag, EvaluateFlag>()
                .Build();
        ...
        private partial struct ReturnFailureJob : IJobEntity
        {
            ...
            public void Execute(ref DynamicBuffer<BranchComponent> branchComponents, ref DynamicBuffer<TaskComponent> taskComponents,
                ref DynamicBuffer<ReturnFailureComponent> returnFailureComponents)
            {
                for (int i = 0; i < returnFailureComponents.Length; ++i)
                {
                    ReturnFailureComponent returnFailureComponent = returnFailureComponents[i];
                    TaskComponent taskComponent = taskComponents[returnFailureComponent.Index];
