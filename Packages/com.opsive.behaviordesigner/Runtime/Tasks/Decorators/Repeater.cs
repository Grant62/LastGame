...
namespace Opsive.BehaviorDesigner.Runtime.Tasks.Decorators
{
    ...
    [Description(@"The repeater task will repeat execution of its child task until the child task has been run a specified number of times. " +
    ...
    public class Repeater : ECSDecoratorTask<RepeaterTaskSystem, RepeaterComponent>, IParentNode, ISavableTask
    {
        ...
        {
            DynamicBuffer<RepeaterComponent> repeaterComponents = world.EntityManager.GetBuffer<RepeaterComponent>(entity);
            RepeaterComponent repeaterComponent = repeaterComponents[m_ComponentIndex];
    ...
    public partial struct RepeaterTaskSystem : ISystem
    {
        ...
        {
            EntityQuery query = SystemAPI.QueryBuilder().WithAllRW<BranchComponent>().WithAllRW<TaskComponent>().WithAllRW<RepeaterComponent>().WithAll<RepeaterFlag, EvaluateFlag>().Build();
        ...
        private partial struct RepeaterJob : IJobEntity
        {
            ...
            public void Execute(ref DynamicBuffer<BranchComponent> branchComponents, ref DynamicBuffer<TaskComponent> taskComponents, ref DynamicBuffer<RepeaterComponent> repeaterComponents)
            {
                ...
                {
                    RepeaterComponent repeaterComponent = repeaterComponents[i];
                    TaskComponent taskComponent = taskComponents[repeaterComponent.Index];
                    ...
                    if (taskComponent.Status == TaskStatus.Queued)
                    {
                        ...
                        DynamicBuffer<RepeaterComponent> repeaterBuffer = repeaterComponents;
                    ...
                    TaskComponent childTaskComponent = taskComponents[taskComponent.Index + 1];
    ...
    public class SharedRepeater : DecoratorNode
    {
        [Tooltip("Should the task be repeated forever?")]
        [SerializeField]
        private SharedVariable<bool> m_RepeatForever = true;
        [Tooltip("The number of times the task should repeat.")]
        [SerializeField]
        private SharedVariable<int> m_RepeatCount;
        [Tooltip("Should the repeater end if the child task fails?")]
        [SerializeField]
        private SharedVariable<bool> m_EndOnFailure;
