...
namespace Opsive.BehaviorDesigner.Runtime.Tasks.Decorators
{
    ...
    public partial struct PriorityEvaluatorSystem : ISystem
    {
        ...
        {
            foreach ((DynamicBuffer<PriorityValueComponent> priorityValueComponents, DynamicBuffer<TaskComponent> taskComponents, Entity entity) in
                     SystemAPI.Query<DynamicBuffer<PriorityValueComponent>, DynamicBuffer<TaskComponent>>().WithAll<PrioritySelectorFlag, EvaluateFlag>().WithEntityAccess())
            {
                ...
                {
                    PriorityValueComponent priorityValueComponent = priorityValueComponents[i];
                    PriorityEvaluator priorityEvaluator = BehaviorTree.GetBehaviorTree(entity).GetTask(priorityValueComponent.Index) as PriorityEvaluator;
                    ...
                    DynamicBuffer<PriorityValueComponent> priorityValueComponentBuffer = priorityValueComponents;
