...
namespace Opsive.BehaviorDesigner.Runtime.Tasks.Decorators
{
    ...
    [Description("The until success task will keep executing its child task until the child task returns success.")]
    ...
    public partial struct UntilSuccessTaskSystem : ISystem
    {
        ...
        {
            EntityQuery query = SystemAPI.QueryBuilder().WithAllRW<BranchComponent>().WithAllRW<TaskComponent>().WithAllRW<UntilSuccessComponent>().WithAll<UntilSuccessFlag, EvaluateFlag>()
                .Build();
        ...
        private partial struct UntilSuccessJob : IJobEntity
        {
            ...
            public void Execute(ref DynamicBuffer<BranchComponent> branchComponents, ref DynamicBuffer<TaskComponent> taskComponents,
                ref DynamicBuffer<UntilSuccessComponent> untilSuccessComponents)
            {
                for (int i = 0; i < untilSuccessComponents.Length; ++i)
                {
                    UntilSuccessComponent untilSuccessComponent = untilSuccessComponents[i];
                    TaskComponent taskComponent = taskComponents[untilSuccessComponent.Index];
