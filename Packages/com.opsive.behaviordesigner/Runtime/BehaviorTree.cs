...
/// ---------------------------------------------
[assembly: InternalsVisibleTo("Opsive.BehaviorDesigner.Editor")]
...
namespace Opsive.BehaviorDesigner.Runtime
{
    ...
    public class BehaviorTree : MonoBehaviour, IGraph, IGraphComponent, ISharedVariableContainer
    {
        ...
        [SerializeField] [Range(1, ushort.MaxValue)]
        private int m_MaxEvaluationCount = 1;
        [Tooltip("A reference to the subtree.")]
        [SerializeField] private Subtree m_Subtree;

        private GameObject m_GameObject;
        private readonly Dictionary<int, int> m_NodeIndexByRuntimeIndex = new();
        private static Dictionary<Entity, BehaviorTree> s_BehaviorTreeByEntity = new();
        private static readonly EventNodeComparer s_EventNodeComparer = new();
        ...
        public TaskStatus Status
        {
            get
            {
                if (!Application.isPlaying || World == null || Entity == null || m_Data == null || m_Data.LogicNodes == null || Data.EventNodes == null)
                {
                ...
                if (startEventNode == null || startEventNode.ConnectedIndex >= m_Data.LogicNodes.Length)
                {
                    return TaskStatus.Inactive;
                }

                // The runtime index will match with the correct Entity TaskComponent.
                ushort runtimeIndex = m_Data.LogicNodes[startEventNode.ConnectedIndex].RuntimeIndex;
                DynamicBuffer<TaskComponent> taskComponents = World.EntityManager.GetBuffer<TaskComponent>(Entity);
        ...
        {
            if (World != null && m_StartWhenEnabled)
            {
                StartBehavior();
            }
        }
        ...
        {
            World world = World == null ? World.DefaultGameObjectInjectionWorld : World;
            Entity entity = Entity == Entity.Null ? world.EntityManager.CreateEntity() : Entity;
        ...
        public bool StartBehavior(World world, Entity entity, Type startBranchType)
        {
            ...
            if (s_BehaviorTreeByEntity.ContainsKey(entity))
            {
                ...
                if (IsPaused(world, entity))
                {
                    ...
                    ITreeLogicNode[] tasks = Data.LogicNodes;
        ...
        internal bool InitializeTree(World world, Entity entity)
        {
            ...
            IEventNode[] eventNodes = m_Data.EventNodes;
...
#if UNITY_EDITOR
            NodeProperties[] eventNodeProperties = Data.EventNodeProperties;
            Array.Sort(eventNodes, eventNodeProperties, s_EventNodeComparer);
            ...
            {
                int index = 0;
                ushort[] disabledEventNodes = m_Data.DisabledEventNodes;
        ...
        private void InitializeBranch(World world, Entity entity, IEventNode eventTask)
        {
            ...
            {
                DynamicBuffer<TaskComponent> taskComponentBuffer = world.EntityManager.AddBuffer<TaskComponent>(entity);
            ...
            ushort startBranchIndex = (ushort)branchComponents.Length;
            ...
            TraversalTaskSystemGroup traversalTaskSystemGroup = world.GetOrCreateSystemManaged<TraversalTaskSystemGroup>();
            ReevaluateTaskSystemGroup reevaluateTaskSystemGroup = world.GetOrCreateSystemManaged<ReevaluateTaskSystemGroup>();
            InterruptTaskSystemGroup interruptTaskSystemGroup = world.GetOrCreateSystemManaged<InterruptTaskSystemGroup>();

            DynamicBuffer<TaskComponent> taskComponents = world.EntityManager.GetBuffer<TaskComponent>(entity);
            ushort taskOffset = (ushort)(eventTask.ConnectedIndex - taskComponents.Length);
            for (int i = eventTask.ConnectedIndex; i < m_Data.LogicNodes.Length; ++i)
            {
                ...
                ushort branchIndex = startBranchIndex;
                if (m_Data.LogicNodes[i].ParentIndex != ushort.MaxValue)
                {
                    branchComponents = world.EntityManager.GetBuffer<BranchComponent>(entity);
                    ushort parentIndex = m_Data.LogicNodes[i].ParentIndex;
                ...
                ITreeLogicNode node = m_Data.LogicNodes[i];
                node.RuntimeIndex = (ushort)(node.Index - taskOffset);
                m_Data.LogicNodes[i] = node;
                if (!m_NodeIndexByRuntimeIndex.ContainsKey(node.RuntimeIndex))
                {
                    // The index will already exist if multiple entities use the same MonoBehaviour.
                ...
                {
                    ComponentType ReevaluateFlag = new();
                    ...
                    ushort parentIndex;
                    ushort compositeParentIndex = m_Data.LogicNodes[i].ParentIndex;
                    do
                    {
                    ...
                    IConditionalAbortParent abortParent = parentComposite as IConditionalAbortParent;
                    if (abortParent != null && abortParent.AbortType != ConditionalAbortType.None)
                    {
                        ushort lowerPriorityLowerIndex = ushort.MaxValue;
                        ushort lowerPriorityUpperIndex = ushort.MaxValue;
                        ...
                        {
                            int parentChildCount = m_Data.GetChildCount(m_Data.LogicNodes[parentIndex], m_Data.LogicNodes);
                            lowerPriorityLowerIndex = AdjustByIndexOffset((ushort)(parentIndex + parentChildCount), taskOffset);
                            ushort parentTranversalIndex = parentIndex;
                            IConditionalAbortParent parentAbortParent = null;
                            while (parentTranversalIndex != ushort.MaxValue && ((parentAbortParent = m_Data.LogicNodes[parentTranversalIndex] as IConditionalAbortParent) != null
                                                                                || m_Data.LogicNodes[parentTranversalIndex] is IDecorator))
                            {
                        ...
                        ushort selfPriorityUpperIndex = ushort.MaxValue;
                        ...
                        DynamicBuffer<ReevaluateTaskComponent> reevaluateTaskComponents = world.EntityManager.AddBuffer<ReevaluateTaskComponent>(entity);
                        ReevaluateTaskComponent reevaluateTaskComponent = new()
                        {
                            Index = node.RuntimeIndex,
        ...
        public bool StartBranch(Type eventTaskType)
        {
            ...
            return StartBranch(World, Entity, eventTaskType);
        }
        ...
        public bool StartBranch(IEventNode eventTask)
        {
            ...
            return StartBranch(World, Entity, eventTask);
        }
        ...
        public bool StartBranch(World world, Entity entity, IEventNode eventTask)
        {
            ...
            ushort connectedIndex = m_Data.LogicNodes[eventTask.ConnectedIndex].RuntimeIndex;
        ...
        internal static bool StartBranch(World world, Entity entity, ushort connectedIndex, bool startEvaluation)
        {
            ...
            TaskComponent startTask = taskComponents[connectedIndex];
            ...
            BehaviorTreeSystemGroup systemGroup = world.GetExistingSystemManaged<BehaviorTreeSystemGroup>();
            if (systemGroup == null)
            {
            ...
            ComponentType activeFlag = taskComponents[connectedIndex].FlagComponentType;
            world.EntityManager.SetComponentEnabled(entity, activeFlag, true);

            DynamicBuffer<BranchComponent> branchComponents = world.EntityManager.GetBuffer<BranchComponent>(entity);
            ushort branchIndex = taskComponents[connectedIndex].BranchIndex;
            BranchComponent branchComponent = branchComponents[branchIndex];
        ...
        {
            if (s_BehaviorTreeByEntity.TryGetValue(entity, out BehaviorTree behaviorTree))
            {
                return behaviorTree;
            }
        ...
        public T[] FindTasks<T>() where T : Task
        {
            ...
            T[] foundTasks = new T[m_Data.LogicNodes.Length];
            int count = FindTasks(foundTasks);
            if (foundTasks.Length != count)
            {
                Array.Resize(ref foundTasks, count);
            }
        ...
        ///     The behavior tree will not be executed instantaneously. It will instead be ticked the next time the DOTS system
        ///     group is updated.
        /// </summary>
        public void Tick()
        {
        ...
        {
            return StopBehavior(World, Entity, pause);
        }
        ...
        public static void StopBehavior(World world, Entity entity)
        {
            ...
            for (int i = 0; i < branchComponents.Length; ++i)
            {
                ...
                ushort taskIndex = branchComponent.ActiveIndex;
                while (taskIndex != ushort.MaxValue)
                {
                    TaskComponent taskComponent = taskComponents[taskIndex];
            ...
            {
                DynamicBuffer<ReevaluateTaskComponent> reevaluateTaskComponents = world.EntityManager.GetBuffer<ReevaluateTaskComponent>(entity);
        ...
        {
            return SetVariableValue(name, value, SharedVariable.SharingScope.Graph);
        }
        ...
        public bool SetVariableValue<T>(PropertyName name, T value, SharedVariable.SharingScope scope)
        {
            ...
            return m_Data.SetVariableValue(this, name, value, scope);
        }
        ...
        public SaveManager.SaveData? Save(SaveManager.VariableSaveScope variableSaveScope = 0)
        {
            ...
            SaveManager.SaveData? saveData = SaveManager.Save(new[] { this }, variableSaveScope);
            if (OnDidSave != null)
            {
        ...
        public bool Save(string filePath, SaveManager.VariableSaveScope variableSaveScope = 0)
        {
            ...
            bool success = SaveManager.Save(new[] { this }, filePath, variableSaveScope);
            if (OnDidSave != null)
            {
        ...
        public bool Load(string filePath, Action<BehaviorTree> afterVariablesRestored = null)
        {
            ...
            bool success = SaveManager.Load(new[] { this }, filePath, afterVariablesRestored);
            if (OnDidLoad != null)
            {
        ...
        public bool Load(SaveManager.SaveData saveData, Action<BehaviorTree> afterVariablesRestored = null)
        {
            ...
            bool success = SaveManager.Load(new[] { this }, saveData, afterVariablesRestored);
            if (OnDidLoad != null)
            {
                OnDidLoad(this, success);
            }
        ...
        {
            MethodInfo method = task.GetType().GetMethod(coroutineName, BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance);
            if (method == null)
            {
            ...
            TaskCoroutine taskCoroutine = new(this, (IEnumerator)method.Invoke(task, new object[] { }), coroutineName);
            if (!m_ActiveTaskCoroutines.TryGetValue(coroutineName, out List<TaskCoroutine> taskCoroutines))
            {
        ...
        public Coroutine StartTaskCoroutine(Task task, string coroutineName, object value)
        {
            ...
            TaskCoroutine taskCoroutine = new(this, (IEnumerator)method.Invoke(task, new[] { value }), coroutineName);
            if (!m_ActiveTaskCoroutines.TryGetValue(coroutineName, out List<TaskCoroutine> taskCoroutines))
            {
                taskCoroutines = new List<TaskCoroutine>();
                m_ActiveTaskCoroutines.Add(coroutineName, taskCoroutines);
        ...
        public void StopAllTaskCoroutines()
        {
            ...
            foreach (KeyValuePair<string, List<TaskCoroutine>> entry in m_ActiveTaskCoroutines)
            {
                List<TaskCoroutine> taskCoroutines = entry.Value;
                for (int i = 0; i < taskCoroutines.Count; ++i)
                {
                    taskCoroutines[i].Stop();
                }
        ...
        private void OnDisable()
        {
            ...
            StopBehavior(World, Entity, m_PauseWhenDisabled);
        }
        ...
        public bool IsNodeActive(bool logicNode, int index)
        {
            ...
            TaskComponent taskComponent = taskComponents[logicNodeIndex];
        ...
        public class BehaviorTreeBaker : Baker<BehaviorTree>
        {
            ...
            public override void Bake(BehaviorTree behaviorTree)
            {
                ...
                Entity entity = GetEntity(behaviorTree, TransformUsageFlags.Dynamic);
                ...
                for (int i = 0; i < worlds.Count; ++i)
                {
                    if (worlds[i].EntityManager.Exists(entity))
                    {
                        ...
                        int connectedIndex = GetStartTaskConnectedIndex(behaviorTree);
                        if (connectedIndex == -1 || connectedIndex == ushort.MaxValue)
                        {
                            return;
                        }

                        DynamicBuffer<TaskComponent> taskComponents = worlds[i].EntityManager.GetBuffer<TaskComponent>(entity);
                        ulong[] tagStableTypeHash = new ulong[taskComponents.Length];
                        ...
                        {
                            DynamicBuffer<ReevaluateTaskComponent> reevaluateTaskComponents = worlds[i].EntityManager.GetBuffer<ReevaluateTaskComponent>(entity);
            ...
            private int GetStartTaskConnectedIndex(BehaviorTree behaviorTree)
            {
                ...
                BehaviorTreeData data = behaviorTree.Data;
            ...
            {
                T systemGroup = world.GetExistingSystemManaged<T>();
                if (systemGroup == null)
                {
                    return null;
                }

                NativeList<SystemHandle> systems = systemGroup.GetAllSystems();
                if (systems.Length == 0)
                {
                    systems.Dispose();
                    return null;
                ...
                string[] systemTypes = new string[systems.Length];
                for (int i = 0; i < systems.Length; ++i)
                {
                    SystemTypeIndex systemTypeIndex = TypeManager.GetSystemTypeIndex((Type)s_GetTypeOfSystemMethod.Invoke(world.Unmanaged, new object[] { systems[i] }));
