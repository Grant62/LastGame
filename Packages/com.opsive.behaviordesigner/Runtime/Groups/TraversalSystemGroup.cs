    ...
    public partial class TraversalSystemGroup : ComponentSystemGroup
    {
        ...
        protected override void OnUpdate()
        {
            ...
            do
            {
                ...
                DetermineEvaluationSystem determineEvaluationSystem = EntityManager.WorldUnmanaged.GetUnsafeSystemRef<DetermineEvaluationSystem>(m_DetermineEvaluationSystemHandle);
                determineEvaluationSystem.Complete(EntityManager);
                evaluate = determineEvaluationSystem.Evaluate;

                EvaluationSystem evaluationSystem = EntityManager.WorldUnmanaged.GetUnsafeSystemRef<EvaluationSystem>(m_EvaluationSystemHandle);
                ...
                if (evaluate)
                {
                    ...
                    {
                        Debug.LogWarning("An infinite loop would have been caused by the TraversalSystemGroup. Please email support@opsive.com with steps to reproduce this error.");
                        break;
                    }
                }
#endif
