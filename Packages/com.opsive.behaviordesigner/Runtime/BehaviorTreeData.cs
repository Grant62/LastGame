...
namespace Opsive.BehaviorDesigner.Runtime
{
    ...
    public class BehaviorTreeData
    {
        ...
        public bool RemoveNode(ITreeLogicNode node)
        {
            ...
            ITreeLogicNode[] dest = new ITreeLogicNode[m_Tasks.Length - 1];
        ...
        public bool RemoveNode(IEventNode eventNode)
        {
            ...
            int index = m_EventTasks.IndexOf(eventNode);
            if (index == -1)
            {
                return false;
            }

            IEventNode[] dest = new IEventNode[m_EventTasks.Length - 1];
        ...
        public void Serialize()
        {
            ...
            if (m_LogicNodeProperties != null && m_Tasks != null && m_LogicNodeProperties.Length <= m_Tasks.Length)
            {
                ...
                {
                    LogicNodeProperties.NodeData nodeData = m_LogicNodeProperties[i].Data;
        ...
        public static Serialization.ValidatedObject ValidateSerializedObject(Type type, FieldInfo field, object value)
        {
            ...
            {
                Type elementType = Serializer.GetElementType(type);
                if (typeof(ILogicNode).IsAssignableFrom(elementType))
                {
                    if (field == null || field.GetCustomAttribute<InspectNodeAttribute>() == null)
                    {
                        IList tasks = value as IList;
                        if (tasks == null)
                        {
                            return new Serialization.ValidatedObject { Type = type, Obj = value };
                        }

                        ushort[] indexValues = new ushort[tasks.Count];
                ...
                else if (Application.isPlaying && (typeof(GameObject).IsAssignableFrom(elementType) || typeof(Component).IsAssignableFrom(elementType)))
                {
                    // Scene objects cannot be serialized at runtime.
            ...
            else if (Application.isPlaying && (typeof(GameObject).IsAssignableFrom(type) || typeof(Component).IsAssignableFrom(type)))
            {
                // Scene objects cannot be serialized at runtime.
                GameObject gameObjectValue = null;
        ...
        public bool Deserialize(IGraphComponent graphComponent, IGraph graph, bool force, bool forceSharedVariables, bool injectSubtrees, bool canDeepCopyVariables = true,
            SharedVariableOverride[] sharedVariableOverrides = null)
        {
            // No need to deserialize if the data is already deserialized.
            if (!force && (m_Tasks != null && m_TaskData != null && m_Tasks.Length == m_TaskData.Length
                           || m_EventTasks != null && m_EventTaskData != null && m_EventTasks.Length == m_EventTaskData.Length))
            {
        ...
        /// <param name="sharedVariableOverrides">A list of SharedVariables that should override the current SharedVariable value.</param>
        /// <returns>True if the tree was deserialized.</returns>
        private bool DeserializeInternal(IGraphComponent graphComponent, IGraph graph, bool force, bool forceSharedVariables, bool injectSubtrees, bool canDeepCopyVariables,
            SharedVariableOverride[] sharedVariableOverrides = null)
        {
            ...
            if (m_TaskData != null && m_TaskData.Length > 0)
            {
                ...
                for (int i = 0; i < m_TaskData.Length; ++i)
                {
                    try
                    {
                        ILogicNode task = m_TaskData[i].DeserializeFields(MemberVisibility.Public, ValidateDeserializedTypeObject,
                            (fieldInfoObj, task, value) =>
                            {
...
#if UNITY_EDITOR
                    // Sanity checks.
                    if (m_Tasks[i].Index >= m_TaskData.Length)
                    {
                        m_Tasks[i].Index = (ushort)i;
                    }

                    if (m_Tasks[i].ParentIndex != ushort.MaxValue && m_Tasks[i].ParentIndex >= m_TaskData.Length)
                    {
                        m_Tasks[i].ParentIndex = ushort.MaxValue;
                    }

                    if (m_Tasks[i].SiblingIndex != ushort.MaxValue && m_Tasks[i].SiblingIndex >= m_TaskData.Length)
                    {
                    ...
                    if (injectSubtrees)
                    {
                        ...
                        if (m_Tasks[i].ParentIndex != ushort.MaxValue && i > 0 && m_Tasks[i - 1] is IParentNode && m_Tasks[i].ParentIndex != m_Tasks[i - 1].Index
                            || m_Tasks[i] is IParentNode && i + 1 == m_Tasks.Length)
                        {
                            Debug.LogError(
                                $"Error: {graph} contains the parent task {m_Tasks[i].GetType().Name} which does not have any children. All parent tasks must contain at least one child.",
                                graph.Parent);
                            errorState = true;
                            continue;
                        }
                        ...
                        if (m_Tasks[i] is ISubtreeReference subtreeReference)
                        {
                            ...
                            Subtree[] subtrees = subtreeReference.Subtrees;
                            if (subtrees != null)
                            {
                                // The parent must be able to accept the number of subtrees that there are.
                                ushort parentIndex = m_Tasks[i].ParentIndex;
                                ...
                                {
                                    Debug.LogError(
                                        $"Error: {graph} on object {graph.Parent} contains multiple subtrees as the starting task or as a child of a parent task which cannot contain so many children (such as a decorator).",
                                        graph.Parent);
                                    errorState = true;
                                    continue;
                                }

                                ITreeLogicNode[][] deserializedNodes = new ITreeLogicNode[subtrees.Length][];
                                for (int j = 0; j < subtrees.Length; ++j)
                                {
                                    ...
                                    if (subtrees[j].SharedVariables != null)
                                    {
                                        ...
                                        int length = subtrees[j].SharedVariables.Length;
                                        int variableCount = 0;
                                        for (int k = 0; k < length; ++k)
                                        {
                                            SharedVariable subtreeVariable = subtrees[j].SharedVariables[k];
                                ...
                                if (!errorState)
                                {
                                    if (m_SubtreeNodesReference == null)
                                    {
                                        m_SubtreeNodesReference = new ResizableArray<SubtreeNodesReference>();
                                    }
            ...
            if (m_EventTaskData != null && m_EventTaskData.Length > 0)
            {
                ...
                {
                    m_EventTasks[i] = m_EventTaskData[i].DeserializeFields(MemberVisibility.Public, ValidateDeserializedTypeObject,
                        (fieldInfoObj, task, value) => { return ValidateDeserializedObject(fieldInfoObj, task, value, ref m_VariableByNameMap, ref taskReferences); }) as IEventNode;
        ...
        {
            FieldInfo fieldInfo = fieldInfoObj as FieldInfo;
            if (fieldInfo == null)
            {
                return value;
            }
            ...
            if (typeof(IList).IsAssignableFrom(type))
            {
                ...
                if (typeof(ILogicNode).IsAssignableFrom(elementType) && fieldInfo.GetCustomAttribute<InspectNodeAttribute>() == null)
                {
                    ...
                    {
                        taskReferences = new ResizableArray<TaskAssignment>();
                    }
                ...
                else if (typeof(SharedVariable).IsAssignableFrom(elementType))
                {
                    ...
                    if (listValue != null)
                    {
                        for (int i = 0; i < listValue.Count; ++i)
                        {
                            SharedVariable sharedVariableElement = listValue[i] as SharedVariable;
                            if (variableByNameMap != null && sharedVariableElement != null && !string.IsNullOrEmpty(sharedVariableElement.Name))
                            {
                                if (variableByNameMap.TryGetValue(new VariableAssignment(sharedVariableElement.Name, sharedVariableElement.Scope), out SharedVariable mappedSharedVariable))
                                {
                                    if (Application.isPlaying && sharedVariableElement.Scope == SharedVariable.SharingScope.Dynamic
                                                              && sharedVariableElement.GetType() != mappedSharedVariable.GetType())
                                    {
                                        Debug.LogError(
                                            $"Error: The dynamic variables with name {sharedVariableElement.Name} have different types. All dynamic variables must have the same type.");
                                ...
                                else if (sharedVariableElement.Scope == SharedVariable.SharingScope.Dynamic)
                                {
                                    ...
                                    Type sharedVariableValueType = sharedVariableElement.GetType().GetGenericArguments()[0];
            ...
            else if (typeof(SharedVariable).IsAssignableFrom(type))
            {
                ...
                {
                    if (variableByNameMap.TryGetValue(new VariableAssignment(sharedVariable.Name, sharedVariable.Scope), out SharedVariable mappedSharedVariable))
                    {
                    ...
                    if (Application.isPlaying && sharedVariable.Scope == SharedVariable.SharingScope.Dynamic)
                    {
                        // New dynamic variables should have the default value.
                        Type sharedVariableValueType = sharedVariable.GetType().GetGenericArguments()[0];
                        if (sharedVariableValueType.IsValueType)
                        {
        ...
        public bool DeserializeSharedVariables(bool force, SharedVariableOverride[] sharedVariableOverrides = null)
        {
            ...
            if (m_SharedVariableData != null && m_SharedVariableData.Length > 0)
            {
                ...
                for (int i = 0; i < m_SharedVariableData.Length; ++i)
                {
                    ...
                    {
                        Serialization unknownSharedVariableData = m_SharedVariableData[i];
                    ...
                    if (Application.isPlaying)
                    {
                        ...
                        SharedVariable overrideVariable = GetOverrideVariable(sharedVariableOverrides, m_SharedVariables[i], true);
        ...
        private static SharedVariable GetOverrideVariable(SharedVariableOverride[] sharedVariableOverrides, SharedVariable graphVariable, bool deserialize)
        {
            ...
            {
                SharedVariable overrideVariable = sharedVariableOverrides[i].Override;
                ...
                SharedVariable sourceVariable = sharedVariableOverrides[i].Source;
        ...
        {
            Dictionary<VariableAssignment, SharedVariable> variableByNameMap = new();
            ...
            {
                GameObjectSharedVariables gameObjectSharedVariablesContainer = parentGameObject.GetComponent<GameObjectSharedVariables>();
            ...
            SceneSharedVariables sceneSharedVariablesContainer = SceneSharedVariables.Instance;
            ...
            ProjectSharedVariables projectSharedVariablesContainer = ProjectSharedVariables.Instance;
        ...
        /// <param name="canDeepCopy">Can the SharedVariables be deep copied?</param>
        /// <param name="variableByNameMap">A reference to the map between the VariableAssignment and SharedVariable.</param>
        private static void PopulateSharedVariablesMapping(IGraph graph, SharedVariable[] sharedVariables, SharedVariable.SharingScope scope, bool canDeepCopy,
            ref Dictionary<VariableAssignment, SharedVariable> variableByNameMap)
        {
...
#if UNITY_EDITOR
                    Debug.LogWarning(
                        "Warning: Multiple SharedVariables with the same name have been added. Please email support@opsive.com with the steps to reproduce this warning. Thank you.");
#endif
                    continue;
                }

                bool deepCopy = canDeepCopy && graph is Subtree && scope == SharedVariable.SharingScope.Graph; // Deep copy variables so the instance is not bound to the subtree.
        ...
        /// <summary>
        ///     When the behavior tree loads not all tasks will be deserialized instantly. TaskA may reference TaskB but TaskB
        ///     hasn't
        ///     been deserialized yet. The TaskAssignment data structure will store all of the references that need to be restored
        ///     after
        ...
        public static void AssignTaskReferences(ILogicNode[] tasks, ResizableArray<TaskAssignment> taskReferences)
        {
            ...
            {
                TaskAssignment taskReference = taskReferences[i];
                Type fieldType = taskReference.Field.FieldType;
                object value = null;

                // The field can be a list or single value.
                if (typeof(IList).IsAssignableFrom(fieldType))
                {
                    IList elements = (IList)taskReferences[i].Value;
                    if (fieldType.IsArray)
                    {
                        // The field type is an array. Create a new array with all of the task instances.
                        ILogicNode[] array = Array.CreateInstance(Serializer.GetElementType(fieldType), elements.Count) as ILogicNode[];
                        for (int j = 0; j < array.Length; ++j)
                        {
                            ushort index = (ushort)elements[j];
                ...
                {
                    // Single ILogicNode value.
                    ushort index = (ushort)taskReference.Value;
                    if (index < tasks.Length)
                    {
                        value = tasks[index];
                    }
        ...
        private void InjectSubtrees()
        {
            ...
            int taskCount = 0;
            int subtreeReferenceCount = 0;
            ResizableArray<SubtreeAssignment> subtreeAssignments = new();
            ushort lastParentIndex = m_Tasks[m_SubtreeNodesReference[0].NodeIndex].ParentIndex;
            int parentIndexOffset = 0;
            for (int i = 0; i < m_SubtreeNodesReference.Count; ++i)
            {
                ISubtreeReference subtreeReference = m_Tasks[m_SubtreeNodesReference[i].NodeIndex] as ISubtreeReference;
                Subtree[] subtrees = subtreeReference.Subtrees;
                if (subtrees != null)
                {
                    ushort indexOffset = 0; // The index offset is relative to each individual ISubtreeReference task.

                    // The parent index will change based on the number of tasks that have been added.
                    ushort parentIndex = m_Tasks[m_SubtreeNodesReference[i].NodeIndex].ParentIndex;
                    ...
                    for (int j = 0; j < subtrees.Length; ++j)
                    {
                        ...
                        (IEventNode, ushort) startNode = subtrees[j].GetEventNode(typeof(Start)); // Returns (IEventNode, index).
                        if (startNode.Item1 == null || startNode.Item1.ConnectedIndex == ushort.MaxValue || !subtrees[j].IsNodeEnabled(false, startNode.Item2))
                        {
                            continue;
                        }

                        ITreeLogicNode firstNode = m_SubtreeNodesReference[i].Nodes[j][startNode.Item1.ConnectedIndex];
                        int subtreeNodeCount = GetChildCount(firstNode, m_SubtreeNodesReference[i].Nodes[j]) + 1; // firstNode should be included in addition to the children.
                    ...
                    if (indexOffset > 0)
                    {
                        // Subtree References may not contain any valid subtrees.
                        subtreeReferenceCount++;
                    }

                    SubtreeNodesReference subtreeNodesReferenceOrig = m_SubtreeNodesReference[i];
            ...
            int targetCount = m_Tasks.Length + taskCount - subtreeReferenceCount;
            int originalTaskCount = m_Tasks.Length;
            ...
            {
                ushort subtreeIndex = (ushort)(subtreeAssignments[i].NodeIndex + addedTasks);
                int subtreeTaskCount = subtreeAssignments[i].NodeCount - (subtreeAssignments[i].IndexOffset == 0 ? 1 : 0);
                if (subtreeTaskCount > 0)
                {
                    // subtreeTaskCount will be zero if a single task replaces the reference task.
                    for (int j = originalTaskCount - 1 + addedTasks; j > subtreeIndex; --j)
                    {
                        ITreeLogicNode node = m_Tasks[j];
                    ...
                    {
                        ushort parentIndex = m_Tasks[subtreeIndex].ParentIndex;
                        while (parentIndex != ushort.MaxValue)
                        {
                            ITreeLogicNode parentNode = m_Tasks[parentIndex];
                    ...
                    if (m_DisabledLogicNodes != null)
                    {
                        for (int j = 0; j < m_DisabledLogicNodes.Length; ++j)
                        {
                            ...
                            {
                                // Remember the last index that was greater than the subtree index so any disabled subtree nodes can be inserted.
                    ...
                    ushort[] subtreeDisabledLogicNodes = subtreeAssignments[i].Subtree.DisabledLogicNodes;
                    ...
                    {
                        int subtreeDisabledLength = subtreeDisabledLogicNodes.Length;
                        ...
                        if (subtreeDisabledLength > 0)
                        {
                            ...
                            int originalLength = m_DisabledLogicNodes.Length - subtreeDisabledLength;
                ...
                SubtreeAssignment subtreeAssignment = subtreeAssignments[i];
            ...
            for (int i = 0; i < subtreeAssignments.Count; ++i)
            {
                ushort subtreeIndex = (ushort)(subtreeAssignments[i].NodeIndex + subtreeAssignments[i].IndexOffset);
                ushort subtreeParentIndex = subtreeAssignments[i].ParentIndex;
#if UNITY_EDITOR
                Vector2 positionOffset = Vector2.zero;
#endif
                for (int j = 0; j < subtreeAssignments[i].NodeCount; ++j)
                {
                    ITreeLogicNode origNode = m_SubtreeNodesReference[subtreeAssignments[i].ReferenceIndex].Nodes[subtreeAssignments[i].SubtreeIndex][j];
                    // The node needs to be copied to prevent the same node from being used in multiple trees.
                    ITreeLogicNode node = CopyUtility.DeepCopy(m_SubtreeNodesReference[subtreeAssignments[i].ReferenceIndex].Nodes[subtreeAssignments[i].SubtreeIndex][j]) as ITreeLogicNode;
...
#if UNITY_EDITOR
                    LogicNodeProperties nodeProperties = CopyUtility.DeepCopy(subtreeAssignments[i].Subtree.LogicNodeProperties[j]) as LogicNodeProperties;
        ...
        public int GetChildCount(ITreeLogicNode node, ITreeLogicNode[] nodes)
        {
            ...
            ITreeLogicNode child = nodes[node.Index + 1];
        ...
        public bool ReevaluateSubtreeReferences(IGraphComponent graphComponent, IGraph graph, Action onBeforeReevaluationSwap)
        {
            ...
            {
                SubtreeNodesReference subtreeNodesReference = m_SubtreeNodesReference[i];
                ISubtreeReference subtreeReference = m_SubtreeNodesReference[i].SubtreeReference;
                subtreeReference.EvaluateSubtrees(graphComponent);
                Subtree[] reevaluatedSubtrees = subtreeReference.Subtrees;
                ...
                {
                    Debug.LogError(
                        "Error: the reevaluated graph contains multiple subtrees as the starting task or as a child of a parent task which cannot contain so many children (such as a decorator).");
                    continue;
                }
                ...
                ushort nodeCount = m_SubtreeNodesReference[i].NodeCount;
                ...
                Subtree[] subtrees = m_SubtreeNodesReference[i].Subtrees;
                int disabledNodesCount = 0;
                for (int j = 0; j < subtrees.Length; ++j)
                {
                ...
                {
                    if (m_DisabledLogicNodes.Length > disabledNodesCount)
                    {
                        // The local tree may not have any disabled nodes.
        ...
        /// </summary>
        /// <param name="graph">
        ///     The graph that the data belongs to.</typeparam>
        ///     <param name="name">
        ///         The name of the SharedVariable that should be retrieved.</typeparam>
        ...
        public SharedVariable GetVariable(IGraph graph, PropertyName name, SharedVariable.SharingScope scope)
        {
            if (m_VariableByNameMap == null)
            {
                DeserializeSharedVariables(false);
                m_VariableByNameMap = PopulateSharedVariablesMapping(graph, true);
            }

            if (m_VariableByNameMap != null && m_VariableByNameMap.TryGetValue(new VariableAssignment(name, scope), out SharedVariable variable))
            {
                return variable;
            }
        ...
        ///     <param name="scope">
        ///         The scope of the SharedVariable that should be set.</typeparam>
        ...
        public bool IsNodeEnabled(bool logicNode, int index)
        {
            ...
            ushort[] disabledNodes = logicNode ? m_DisabledLogicNodes : m_DisabledEventNodes;
