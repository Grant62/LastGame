...
namespace Opsive.BehaviorDesigner.Runtime.Utility
{
    ...
    public static class SaveManager
    {
        ...
        public static SaveData? Save(BehaviorTree[] behaviorTrees, VariableSaveScope variableSaveScope = 0)
        {
            ...
            List<VariableSaveData> variableSaveDataList = new();
            HashSet<GameObject> savedGameObjectVariables = null;
            int behaviorTreeSaveCount = 0;
            for (int i = 0; i < behaviorTrees.Length; ++i)
            {
                ...
                BehaviorTreeSaveData? behaviorTreeSaveData = Save(behaviorTrees[i]);
                if (behaviorTreeSaveData.HasValue)
                {
                    ...
                    if ((variableSaveScope & VariableSaveScope.GameObjectVariables) != 0)
                    {
                        ...
                        GameObjectSharedVariables gameObjectSharedVariables = behaviorTrees[i].gameObject.GetComponent<GameObjectSharedVariables>();
                        if (gameObjectSharedVariables != null)
                        {
                            VariableSaveData? variableSaveData = Save(gameObjectSharedVariables);
            ...
            {
                BehaviorTreeSaveData[] behaviorTreesSaveData = saveData.BehaviorTreeSaveData;
                Array.Resize(ref behaviorTreesSaveData, behaviorTreeSaveCount);
            ...
            if ((variableSaveScope & VariableSaveScope.SceneVariables) != 0)
            {
                ...
                {
                    VariableSaveData? variableSaveData = Save(SceneSharedVariables.Instance);
                    if (variableSaveData.HasValue)
                    {
                        variableSaveDataList.Add(variableSaveData.Value);
                    }
            ...
            if ((variableSaveScope & VariableSaveScope.ProjectVariables) != 0)
            {
                ...
                {
                    VariableSaveData? variableSaveData = Save(ProjectSharedVariables.Instance);
                    if (variableSaveData.HasValue)
                    {
                        variableSaveDataList.Add(variableSaveData.Value);
                    }
        ...
        {
            SaveData? saveData = Save(behaviorTrees, variableSaveScope);
        ...
        private static BehaviorTreeSaveData? Save(BehaviorTree behaviorTree)
        {
            ...
            DynamicBuffer<TaskComponent> taskComponents = behaviorTree.World.EntityManager.GetBuffer<TaskComponent>(behaviorTree.Entity);
            Serialization[] serializedTaskComponents = new Serialization[taskComponents.Length];
            ...
            DynamicBuffer<BranchComponent> branchComponents = behaviorTree.World.EntityManager.GetBuffer<BranchComponent>(behaviorTree.Entity);
            Serialization[] serializedBranchComponents = new Serialization[branchComponents.Length];
            ...
            {
                DynamicBuffer<ReevaluateTaskComponent> reevaluateTaskComponents = behaviorTree.World.EntityManager.GetBuffer<ReevaluateTaskComponent>(behaviorTree.Entity);
            ...
            ILogicNode[] tasks = behaviorTree.LogicNodes;
            List<TaskSaveData> serializedTaskData = new();
            for (int i = 0; i < tasks.Length; ++i)
            {
                ...
                ISavableTask saveableTask = tasks[i] as ISavableTask;
                MemberVisibility reflectionType = saveableTask.GetSaveReflectionType(-1);
                if (reflectionType != MemberVisibility.None)
                {
                    Serialization serializedData = Serialization.Serialize(saveableTask, reflectionType, ValidateSerializedObject);
                    serializedTaskData.Add(new TaskSaveData { Value = new[] { serializedData } });
                }
                else
                {
                    object taskData = saveableTask.Save(behaviorTree.World, behaviorTree.Entity);
                    if (taskData == null)
                    {
                        continue;
                    }
                    ...
                    {
                        IList taskDataList = taskData as IList;
                        Serialization[] taskSaveData = new Serialization[taskDataList.Count];
                    ...
                    {
                        Serialization[] serializedValue = { taskData is Serialization ? taskData as Serialization : Serialization.Serialize(taskData) };
                        serializedTaskData.Add(new TaskSaveData { Value = serializedValue });
                    }
                }
            }

            BehaviorTreeSaveData behaviorTreeSaveData = new()
            {
            ...
            VariableSaveData? variableSaveData = Save(behaviorTree as ISharedVariableContainer);
            if (variableSaveData.HasValue)
            {
        ...
        private static VariableSaveData? Save(ISharedVariableContainer variableContainer)
        {
            ...
            SharedVariable[] sharedVariables = variableContainer.SharedVariables;
            ...
            if (sharedVariables != null)
            {
                ...
                {
                    serializedSharedVariablesData[i] = Serialization.Serialize(sharedVariables[i].GetValue(), ValidateSerializedObject);
                }
            }
        ...
        public static bool Load(BehaviorTree[] behaviorTrees, SaveData saveData, Action<BehaviorTree> afterVariablesRestored = null)
        {
            ...
            bool loadedSceneVariables = false;
            bool loadedProjectVariables = false;
            int loadCount = 0;
            ...
            if (saveData.BehaviorTreeSaveData != null && saveData.BehaviorTreeSaveData.Length > 0)
            {
                ...
                Dictionary<int, BehaviorTreeSaveData> behaviorTreeSaveDataByID = new();
                ...
                for (int i = 0; i < behaviorTrees.Length; ++i)
                {
                    ...
                    if (behaviorTreeSaveDataByID.TryGetValue(behaviorTree.UniqueID, out BehaviorTreeSaveData behaviorTreeSaveData))
                    {
                        // Restore the GameObjectSharedVariables if they haven't already been restored.
                        if (variableSaveDataByID != null)
                        {
                            GameObjectSharedVariables gameObjectSharedVariables = behaviorTree.GetComponent<GameObjectSharedVariables>();
                            if (gameObjectSharedVariables != null)
                            {
                                if (variableSaveDataByID.TryGetValue(gameObjectSharedVariables.UniqueID, out VariableSaveData variableSaveData))
                                {
                        ...
                        Dictionary<VariableAssignment, SharedVariable> variableByNameMap = PopulateSharedVariablesMapping(behaviorTree, true);
        ...
        private static bool Load(BehaviorTree behaviorTree, BehaviorTreeSaveData saveData, Action<BehaviorTree> afterVariablesRestored,
            Dictionary<VariableAssignment, SharedVariable> variableByNameMap)
        {
            // The ID must match.
            ...
            bool enableEntity = behaviorTree.World.EntityManager.HasComponent<EnabledFlag>(behaviorTree.Entity)
                                && behaviorTree.World.EntityManager.IsComponentEnabled<EnabledFlag>(behaviorTree.Entity);
            bool evaluateEntity = behaviorTree.World.EntityManager.HasComponent<EvaluateFlag>(behaviorTree.Entity)
                                  && behaviorTree.World.EntityManager.IsComponentEnabled<EvaluateFlag>(behaviorTree.Entity);
            bool active = behaviorTree.IsActive();
            if (active)
            {
            ...
            if (behaviorTree.World.EntityManager.HasBuffer<ReevaluateTaskComponent>(behaviorTree.Entity))
            {
                DynamicBuffer<ReevaluateTaskComponent> reevaluatedTaskComponents = behaviorTree.World.EntityManager.GetBuffer<ReevaluateTaskComponent>(behaviorTree.Entity);
            ...
            for (int i = 0; i < tasks.Length; ++i)
            {
                ...
                TaskSaveData taskSaveData = saveData.TaskData[saveDataIndex];
                saveDataIndex++;
                ...
                if (reflectionType != MemberVisibility.None)
                {
                    taskSaveData.Value[0].DeserializeFields(saveableTask, saveableTask.GetSaveReflectionType(0), ValidateDeserializedTypeObject,
                        (fieldInfoObj, task, value) => { return ValidateDeserializedObject(fieldInfoObj, task, value, ref variableByNameMap, ref taskReferences); });
                }
                else
                {
                    if (taskSaveData.Value.Length == 1)
                    {
                        ...
                        {
                            saveableTask.Load(taskSaveData.Value[0].DeserializeFields(MemberVisibility.Public, ValidateDeserializedTypeObject,
                                    (fieldInfoObj, task, value) => { return ValidateDeserializedObject(fieldInfoObj, task, value, ref variableByNameMap, ref taskReferences); }),
                                behaviorTree.World,
                                behaviorTree.Entity, variableByNameMap, ref taskReferences);
                        }
                    }
                    else
                    {
                        object[] taskData = new object[taskSaveData.Value.Length];
                        for (int j = 0; j < taskSaveData.Value.Length; ++j)
                        {
                            ...
                            {
                                taskData[j] = taskSaveData.Value[j].DeserializeFields(MemberVisibility.Public, ValidateDeserializedTypeObject,
                                    (fieldInfoObj, task, value) => { return ValidateDeserializedObject(fieldInfoObj, task, value, ref variableByNameMap, ref taskReferences); });
                            }
                        }
            ...
            AssignTaskReferences(behaviorTree.LogicNodes, taskReferences);
        ...
        private static bool Load(ISharedVariableContainer variableContainer, VariableSaveData saveData)
        {
            ...
            if (sharedVariables != null)
            {
                for (int i = 0; i < saveData.Values.Length; ++i)
                {
                    ...
                    object sharedVariableValue = saveData.Values[i].DeserializeFields(MemberVisibility.Public);
