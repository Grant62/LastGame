    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            pixel_t VertShader(vertex_t input)
            {
                ...
                if (UNITY_MATRIX_P[3][3] == 0) scale = lerp(abs(scale) * (1 - _PerspectiveFilter),
                                             scale,
            ...
            fixed4 PixShader(pixel_t input) : SV_Target
            {
                ...
                float3 bump = UnpackNormal(tex2D(_BumpMap,
                                                 input.textures.xy + float2(_FaceUVSpeedX, _FaceUVSpeedY) * _Time.y)).xyz;
                ...
                #if UNDERLAY_INNER
                float d = tex2D(_MainTex, input.texcoord2.xy).a * input.texcoord2.z;
                faceColor += input.underlayColor * (1 - saturate(d - input.texcoord2.w)) * saturate(1 - sd) * (1 - faceColor.
                    a);
                #endif
