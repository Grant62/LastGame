...
namespace Cysharp.Threading.Tasks.Editor
{
    public class UniTaskTrackerWindow : EditorWindow
    {
        ...
        #region HeadPanel
        public static bool EnableAutoReload { get => TaskTracker.EditorEnableState.EnableAutoReload; }

        public static bool EnableTracking { get => TaskTracker.EditorEnableState.EnableTracking; }

        public static bool EnableStackTrace { get => TaskTracker.EditorEnableState.EnableStackTrace; }
        ...
        private static readonly GUIContent EnableTrackingHeadContent = EditorGUIUtility.TrTextContent("Enable Tracking", "Start to track async/await UniTask. Performance impact: low");
        ...
        private void RenderDetailsPanel()
        {
            ...
            if (selected.Count > 0)
            {
                ...
                UniTaskTrackerViewItem item = treeView.CurrentBindingItems.FirstOrDefault(x => x.id == first) as UniTaskTrackerViewItem;
                if (item != null)
                {
            ...
            EditorGUILayout.SelectableLabel(message, detailsStyle, GUILayout.ExpandHeight(true), GUILayout.ExpandWidth(true), GUILayout.MinWidth(vector.x), GUILayout.MinHeight(vector.y));
            EditorGUILayout.EndScrollView();
        }
        #endregion
    }
