...
namespace Cysharp.Threading.Tasks
{
    ...
    public static class TaskTracker
    {
        ...
        private static List<KeyValuePair<IUniTaskSource, (string formattedType, int trackingId, DateTime addTime, string stackTrace)>> listPool = new();

        private static readonly WeakDictionary<IUniTaskSource, (string formattedType, int trackingId, DateTime addTime, string stackTrace)> tracking = new();
        ...
        public static void ForEachActiveTask(Action<int, string, UniTaskStatus, DateTime, string> action)
        {
            lock (listPool)
            {
                int count = tracking.ToList(ref listPool, false);
                try
                {
