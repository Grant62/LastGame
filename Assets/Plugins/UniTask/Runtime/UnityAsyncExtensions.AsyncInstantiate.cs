...
namespace Cysharp.Threading.Tasks
{
    public static class AsyncInstantiateOperationExtensions
    {
            ...
            CancellationToken cancellationToken = default, bool cancelImmediately = false)
        {
            ...
            return new UniTask<Object[]>(AsyncInstantiateOperationConfiguredSource.Create(asyncOperation, timing, progress, cancellationToken, cancelImmediately, out short token), token);
        }
            ...
            where T : Object
        {
            ...
            if (asyncOperation.isDone) return UniTask.FromResult(asyncOperation.Result);
            return new UniTask<T[]>(AsyncInstantiateOperationConfiguredSource<T>.Create(asyncOperation, timing, progress, cancellationToken, cancelImmediately, out short token), token);
        }
        ...
        private sealed class AsyncInstantiateOperationConfiguredSource : IUniTaskSource<Object[]>, IPlayerLoopItem, ITaskPoolNode<AsyncInstantiateOperationConfiguredSource>
        {
            ...
            public ref AsyncInstantiateOperationConfiguredSource NextNode { get => ref nextNode; }
            ...
            public static IUniTaskSource<Object[]> Create(AsyncInstantiateOperation asyncOperation, PlayerLoopTiming timing, IProgress<float> progress, CancellationToken cancellationToken,
                bool cancelImmediately, out short token)
            {
                ...
                if (!pool.TryPop(out AsyncInstantiateOperationConfiguredSource result))
                {
                ...
                if (cancelImmediately && cancellationToken.CanBeCanceled)
                {
                    ...
                    {
                        AsyncInstantiateOperationConfiguredSource source = (AsyncInstantiateOperationConfiguredSource)state;
            ...
            where T : Object
        {
            ...
            public ref AsyncInstantiateOperationConfiguredSource<T> NextNode { get => ref nextNode; }
            ...
            public static IUniTaskSource<T[]> Create(AsyncInstantiateOperation<T> asyncOperation, PlayerLoopTiming timing, IProgress<float> progress, CancellationToken cancellationToken,
                bool cancelImmediately, out short token)
            {
                if (cancellationToken.IsCancellationRequested)
                {
                ...
                if (!pool.TryPop(out AsyncInstantiateOperationConfiguredSource<T> result))
                {
                ...
                if (cancelImmediately && cancellationToken.CanBeCanceled)
                {
                    result.cancellationTokenRegistration = cancellationToken.RegisterWithoutCaptureExecutionContext(state =>
                    {
                        AsyncInstantiateOperationConfiguredSource<T> source = (AsyncInstantiateOperationConfiguredSource<T>)state;
                        source.core.TrySetCanceled(source.cancellationToken);
                    }, result);
                }
