            ...
            public WhenAnyLRPromise(UniTask<T> leftTask, UniTask rightTask)
            {
                ...
                RIGHT:
                {
                    ...
                    else
                    {
                        awaiter.SourceOnCompleted(state =>
                        {
                            using (StateTuple<WhenAnyLRPromise<T>, Awaiter> t = (StateTuple<WhenAnyLRPromise<T>, Awaiter>)state)
                            {
            ...
            public WhenAnyPromise(UniTask[] tasks, int tasksLength)
            {
                ...
                for (int i = 0; i < tasksLength; i++)
                {
                    ...
                    else
                    {
                        awaiter.SourceOnCompleted(state =>
                        {
                            using (StateTuple<WhenAnyPromise, Awaiter, int> t = (StateTuple<WhenAnyPromise, Awaiter, int>)state)
                            {
                                TryInvokeContinuation(t.Item1, t.Item2, t.Item3);
                            }
                        }, StateTuple.Create(this, awaiter, i));
