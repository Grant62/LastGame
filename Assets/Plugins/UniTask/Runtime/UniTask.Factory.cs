...
namespace Cysharp.Threading.Tasks
{
    ...
    {
        private static readonly UniTask CanceledUniTask = new Func<UniTask>(() => { return new UniTask(new CanceledResultSource(CancellationToken.None), 0); })();
        ...
        /// <summary>
        ///     Create async void(UniTaskVoid) UnityAction.
        ///     For example: onClick.AddListener(UniTask.UnityAction(async (T0 arg0, T1 arg1, T2 arg2, CancellationToken
        ///     cancellationToken) => { /* */ } ))
        /// </summary>
        ...
        /// <summary>
        ///     Create async void(UniTaskVoid) UnityAction.
        ///     For example: onClick.AddListener(UniTask.UnityAction(async (T0 arg0, T1 arg1, T2 arg2, T3 arg3, CancellationToken
        ///     cancellationToken) => { /* */ } ))
        /// </summary>
        ...
        private sealed class DeferPromiseWithState<TState> : IUniTaskSource
        {
            ...
            private readonly TState argument;
            private UniTask task;
            private Awaiter awaiter;
            ...
            public UniTaskStatus GetStatus(short token)
            {
                Func<TState, UniTask> f = Interlocked.Exchange(ref factory, null);
                if (f != null)
                {
                    task = f(argument);
                    awaiter = task.GetAwaiter();
        ...
        private sealed class DeferPromiseWithState<TState, TResult> : IUniTaskSource<TResult>
        {
            ...
            public UniTaskStatus GetStatus(short token)
            {
                Func<TState, UniTask<TResult>> f = Interlocked.Exchange(ref factory, null);
                if (f != null)
                {
                    task = f(argument);
                    awaiter = task.GetAwaiter();
