...
namespace Cysharp.Threading.Tasks
{
    public partial struct UniTask
    {
        ...
        public static UniTask<T[]> WhenAll<T>(IEnumerable<UniTask<T>> tasks)
        {
            ...
            {
                WhenAllPromise<T> promise = new(span.Array, span.Length); // consumed array in constructor.
        ...
        public static UniTask WhenAll(IEnumerable<UniTask> tasks)
        {
            ...
            {
                WhenAllPromise promise = new(span.Array, span.Length); // consumed array in constructor.
            ...
            public WhenAllPromise(UniTask[] tasks, int tasksLength)
            {
                ...
                for (int i = 0; i < tasksLength; i++)
                {
                    ...
                    else
                    {
                        awaiter.SourceOnCompleted(state =>
                        {
                            using (StateTuple<WhenAllPromise, Awaiter> t = (StateTuple<WhenAllPromise, Awaiter>)state)
                            {
