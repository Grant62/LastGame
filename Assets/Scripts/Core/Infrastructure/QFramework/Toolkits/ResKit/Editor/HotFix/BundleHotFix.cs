...
namespace QFramework
{
    ...
    {
        private static readonly string m_BunleTargetPath = Application.streamingAssetsPath + AssetBundleSettings.RELATIVE_AB_ROOT_FOLDER;
        private static readonly string m_HotPath = Application.dataPath + "/../Hot/" + EditorUserBuildSettings.activeBuildTarget;
        ...
        private static void ReadMD5Com(string abmd5Path, string hotCount)
        {
            ...
            using (StreamReader streamReader = new(abmd5Path))
            {
                JsonUtility.FromJson<List<ABMD5>>(streamReader.ReadToEnd()).ForEach(_ => { m_PackedMd5.Add(_.ABName, _); });
            }

            List<string> changeList = new();
            DirectoryInfo directory = new(m_BunleTargetPath);
        ...
        private static void CopyAbAndGeneratJson(List<string> changeList, string hotCount)
        {
            ...
            //生成服务器Patch
            DirectoryInfo directory = new(m_HotPath);
            FileInfo[] files = directory.GetFiles("*", SearchOption.AllDirectories);
            Pathces pathces = new();
            pathces.Version = 1;
        ...
        {
            DirectoryInfo directoryInfo = new(m_BunleTargetPath);
