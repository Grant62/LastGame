...
")]
        ...
        public void Recycle2Cache()
        {
            ...
            {
                foreach (Object o in mObject2Unload)
                {
                    if (o)
                    {
        ...
        public IRes LoadResSync(ResSearchKeys resSearchKeys)
        {
            ...
            IRes res = ResMgr.Instance.GetRes(resSearchKeys);
            if (res == null)
            {
        ...
        public float Progress
        {
            get
            {
                ...
                float currentValue = unit * (mResList.Count - mLoadingCount);
        ...
        public void Add2Load(ResSearchKeys resSearchKeys, Action<bool, IRes> listener = null,
            bool lastOrder = true)
        {
            IRes res = FindResInArray(mResList, resSearchKeys);
            if (res != null)
            {
                if (listener != null)
                {
            ...
            if (depends != null)
            {
                ...
                {
                    ResSearchKeys searchRule = ResSearchKeys.Allocate(depend, null, typeof(AssetBundle));
        ...
        public Object LoadAssetSync(ResSearchKeys resSearchKeys)
        {
            ...
            if (resSearchKeys.AssetType == typeof(Sprite))
            {
                if (AssetBundlePathHelper.SimulationMode)
                {
                    ...
                    Texture2D texture = LoadResSync(resSearchKeys).Asset as Texture2D;
        ...
        public void ReleaseRes(string resName)
        {
            ...
            if (AssetBundlePathHelper.SimulationMode)
            {
                ...
                {
                    Sprite sprite = mCachedSpriteDict[resName];
                    Object.Destroy(sprite);
                    mCachedSpriteDict.Remove(resName);
                }
            }

            ResSearchKeys resSearchRule = ResSearchKeys.Allocate(resName);

            IRes res = ResMgr.Instance.GetRes(resSearchRule);
        ...
        public void ReleaseAllRes()
        {
            if (AssetBundlePathHelper.SimulationMode)
            {
                foreach (KeyValuePair<string, Sprite> spritePair in mCachedSpriteDict)
                {
        ...
        private void DoLoadAsync()
        {
            ...
            LinkedListNode<IRes> nextNode = mWaitLoadList.First;
        ...
        private void RemoveCallback(IRes res, bool release)
        {
            ...
            {
                LinkedListNode<CallBackWrap> current = mCallbackRecordList.First;
        ...
        private void RemoveAllCallbacks(bool release)
        {
            if (mCallbackRecordList != null)
            {
                int count = mCallbackRecordList.Count;
                while (count > 0)
                {
                    --count;
                    if (release)
        ...
        {
            ResSearchKeys searchRule = ResSearchKeys.Allocate(res.AssetName, res.OwnerBundleName, res.AssetType);

            //再次确保队列中没有它
            IRes oldRes = FindResInArray(mResList, searchRule);
