...
namespace QFramework
{
    public class AssetDataTable : Table<AssetData>
    {
        ...
        {
            string assetName = resSearchKeys.AssetName.ToLower();

            IEnumerable<AssetData> assetDatas = NameIndex
                .Get(assetName);
            ...
            {
                short assetTypeCode = resSearchKeys.AssetType.ToCode();

                if (assetTypeCode == 0) { }
                else
                {
                    IEnumerable<AssetData> newAssetDatas = assetDatas.Where(a => a.AssetObjectTypeCode == assetTypeCode);
