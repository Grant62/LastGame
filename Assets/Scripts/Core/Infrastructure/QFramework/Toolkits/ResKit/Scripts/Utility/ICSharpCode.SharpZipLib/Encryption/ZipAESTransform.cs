...
namespace ICSharpCode.SharpZipLib.Encryption
{
    ...
    internal class ZipAESTransform : ICryptoTransform
    {
        ...
        public ZipAESTransform(string key, byte[] saltBytes, int blockSize, bool writeMode)
        {
            ...
            _encryptBuffer = new byte[InputBlockSize];
            _encrPos = ENCRYPT_BLOCK;

            // Performs the equivalent of derive_key in Dr Brian Gladman's pwd2key.c
            Rfc2898DeriveBytes pdb = new(key, saltBytes, KEY_ROUNDS);
            Aes rm = Aes.Create();
            rm.Mode = CipherMode.ECB; // No feedback from cipher for CTR mode
            _counterNonce = new byte[InputBlockSize];
        ...
        public int TransformBlock(byte[] inputBuffer, int inputOffset, int inputCount, byte[] outputBuffer, int outputOffset)
        {
            ...
            while (ix < inputCount)
            {
                if (_encrPos == ENCRYPT_BLOCK)
                {
                    ...
                    _encryptor.TransformBlock(_counterNonce, 0, InputBlockSize, _encryptBuffer, 0);
                    _encrPos = 0;
                }
