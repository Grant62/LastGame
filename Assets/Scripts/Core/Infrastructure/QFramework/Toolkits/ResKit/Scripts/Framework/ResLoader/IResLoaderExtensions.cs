...
")]
        ...
        public static T LoadSync<T>(this IResLoader self, string assetName) where T : Object
        {
            ...
            {
                ResSearchKeys resSearchKeys = ResSearchKeys.Allocate(assetName, null, GameObjectType);
                T retAsset = (self.LoadAssetSync(resSearchKeys) as GameObject)?.GetComponent<T>();
                resSearchKeys.Recycle2Cache();
                return retAsset;
            }
            else
            {
                ResSearchKeys resSearchKeys = ResSearchKeys.Allocate(assetName, null, type);
                T retAsset = self.LoadAssetSync(resSearchKeys) as T;
                resSearchKeys.Recycle2Cache();
                return retAsset;
            }
        }
        ...
        public static T LoadSync<T>(this IResLoader self, string ownerBundle, string assetName) where T : Object
        {
            ...
            if (ComponentType.IsAssignableFrom(type))
            {
                ResSearchKeys resSearchKeys = ResSearchKeys.Allocate(assetName, ownerBundle, GameObjectType);
                T retAsset = (self.LoadAssetSync(resSearchKeys) as GameObject)?.GetComponent<T>();
                resSearchKeys.Recycle2Cache();
                return retAsset;
            }
            ...
            {
                ResSearchKeys resSearchKeys = ResSearchKeys.Allocate(assetName, ownerBundle, type);
                T retAsset = self.LoadAssetSync(resSearchKeys) as T;
                resSearchKeys.Recycle2Cache();
                return retAsset;
            }
...
")]
        ...
        public static void Add2Load(this IResLoader self, string assetName, Action<bool, IRes> listener = null,
            bool lastOrder = true)
        {
            ResSearchKeys searchRule = ResSearchKeys.Allocate(assetName);
            ...
            bool lastOrder = true)
        {
            ...
            {
                ResSearchKeys searchRule = ResSearchKeys.Allocate(assetName, null, type);
                self.Add2Load(searchRule, listener, lastOrder);
                searchRule.Recycle2Cache();
            }
        }
        ...
        public static void Add2Load(this IResLoader self, string ownerBundle, string assetName,
            ...
            bool lastOrder = true)
        {
            ResSearchKeys searchRule = ResSearchKeys.Allocate(assetName, ownerBundle);
            self.Add2Load(searchRule, listener, lastOrder);
            searchRule.Recycle2Cache();
        }
            ...
            bool lastOrder = true)
        {
            ...
            {
                ResSearchKeys searchRule = ResSearchKeys.Allocate(assetName, ownerBundle, type);
                self.Add2Load(searchRule, listener, lastOrder);
                searchRule.Recycle2Cache();
            }
        }
...
")]
        ...
        {
            ResSearchKeys resSearchRule = ResSearchKeys.Allocate(assetName);
        ...
        public static void LoadSceneSync(this IResLoader self, string ownerBundle, string assetName,
            ...
            LocalPhysicsMode physicsMode = LocalPhysicsMode.None)
        {
            ResSearchKeys resSearchRule = ResSearchKeys.Allocate(assetName, ownerBundle);
            self.LoadSceneSync(resSearchRule, mode, physicsMode);
            resSearchRule.Recycle2Cache();
        }
...
")]
        ...
        {
            ResSearchKeys resSearchKey = ResSearchKeys.Allocate(sceneName);
        ...
        public static void LoadSceneAsync(this IResLoader self, string bundleName, string sceneName,
            ...
            Action<AsyncOperation> onStartLoading = null)
        {
            ResSearchKeys resSearchKey = ResSearchKeys.Allocate(bundleName, sceneName);
            self.LoadSceneAsync(resSearchKey, loadSceneMode, physicsMode, onStartLoading);
            resSearchKey.Recycle2Cache();
        }
            ...
            Action<AsyncOperation> onStartLoading = null)
        {
            ...
            {
                //加载的为场景
                IRes res = ResFactory.AssetBundleSceneResCreator.Create(resSearchKeys);
#if UNITY_EDITOR
                if (AssetBundlePathHelper.SimulationMode)
                {
                    string path =
                    ...
                    if (!string.IsNullOrEmpty(path))
                    {
                        LoadSceneParameters sceneParameters = new()
                        {
                            loadSceneMode = loadSceneMode,
                            localPhysicsMode = physicsMode
                        };

                        AsyncOperation asyncOperation = EditorSceneManager.LoadSceneAsyncInPlayMode(
                            path,
                            sceneParameters);
...
#endif
                {
                    string sceneName = resSearchKeys.OriginalAssetName;
