...
namespace QFramework
{
    ...
    {
        public static HotPatchManager Instance { get => SingletonProperty<HotPatchManager>.Instance; }

        private HotPatchManager() { }

        private MonoBehaviour m_Mono;

        private readonly string m_UnPackPath = Application.persistentDataPath + "/Origin";
        /// <summary>
        ///     热更资源加载目录，请与reskit加载目录保持一致
        /// </summary>
        private readonly string m_DownLoadPath = Application.persistentDataPath + "/DownLoad";
        public string CurVersion { get; private set; }

        private string m_CurPackName;
        private readonly string m_ServerXmlPath = Application.persistentDataPath + "/ServerInfo.xml";
        private readonly string m_LocalXmlPath = Application.persistentDataPath + "/LocalInfo.xml";
        ...
        //所有热更的东西
        private readonly Dictionary<string, Patch> m_HotFixDic = new();
        //所有需要下载的东西
        private readonly List<Patch> m_DownLoadList = new();
        //所有需要下载的东西的Dic
        private readonly Dictionary<string, Patch> m_DownLoadDic = new();
        //服务器上的资源名对应的MD5，用于下载后MD5校验
        private readonly Dictionary<string, string> m_DownLoadMD5Dic = new();
        //计算需要解压的文件
        private readonly List<string> m_UnPackedList = new();
        //原包记录的MD5码
        private readonly Dictionary<string, ABMD5> m_PackedMd5 = new();
        //服务器列表获取错误回调
        ...
        //储存已经下载的资源
        public List<Patch> m_AlreadyDownList = new();
        //是否开始下载
        public bool StartDownload;
        //尝试重新下载次数
        private int m_TryDownCount;
        ...
        private void ReadMD5()
        {
            ...
            JsonUtility.FromJson<List<ABMD5>>(md5.text).ForEach(_ => { m_PackedMd5.Add(_.ABName, _); });
        }
        ...
        /// <param name="callBack"></param>
        /// <returns></returns>
        private IEnumerator UnPackToPersistentDataPath(Action callBack)
        {
            foreach (string fileName in m_UnPackedList)
            {
        ...
        private bool CheckLocalAndServerPatch()
        {
            ...
            if (localGameVesion != null && m_GameVersion.Pathces != null && localGameVesion.Pathces != null && m_GameVersion.Pathces.Length > 0
                && m_GameVersion.Pathces[m_GameVersion.Pathces.Length - 1].Version != localGameVesion.Pathces[localGameVesion.Pathces.Length - 1].Version)
                return true;
        ...
        public IEnumerator StartDownLoadAB(Action callBack, List<Patch> allPatch = null)
        {
            ...
            List<DownLoadAssetBundle> downLoadAssetBundles = new();
