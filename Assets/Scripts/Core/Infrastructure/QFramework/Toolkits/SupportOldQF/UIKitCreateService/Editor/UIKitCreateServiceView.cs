...
using System.IO;
using Core.Infrastructure.QFramework.Toolkits.SupportOldQF.UIKitCreateService;
using UnityEditor;
...
namespace QFramework
{
    public class UIKitCreateServiceView
    {
        ...
        private string mPrefabGeneratePath { get => $"{mModuleFolder}{UIKitSettingData.Load().UIPrefabDir}/{mPanelNameToCreate}.prefab"; }

        private string mSceneGeneratePath
        {
            get => $"{mModuleFolder}/Scenes/TestUIPanels/{mPanelNameToCreate}".GetFolderPath() +
                   $"/Test{mPanelNameToCreate.GetFileNameWithoutExtend()}.unity";
        }

        private string mMainCodeFileGenreatePath
        {
            get => mPrefabGeneratePath.Replace(UIKitSettingData.Load().UIPrefabDir,
                    UIKitSettingData.Load().UIScriptDir)
                .Replace(".prefab", ".cs");
        }

        private string mDesignerCodeFileGenreatePath
        {
            get => mPrefabGeneratePath.Replace(UIKitSettingData.Load().UIPrefabDir,
                    UIKitSettingData.Load().UIScriptDir)
        ...
        {
            GUIStyle labelStyle = new(GUI.skin.GetStyle("label"))
            {
                richText = true
            };
            return labelStyle;
        ...
        public void OnGUI()
        {
            ...
            if (GUILayout.Button("..."))
            {
                string folderPath = EditorUtility.OpenFolderPanel(LocaleText.ModuleFolder, mModuleFolder, "Assets");
        ...
        {
            string panelName = mPanelNameToCreate;

            if (!string.IsNullOrEmpty(panelName))
            {
                string fullScenePath = $"{mModuleFolder}/Scenes/TestUIPanels/{panelName}".GetFolderPath()
                    .CreateDirIfNotExists()
                    .Builder()
                    .Append("/Test{0}.unity".FillFormat(panelName.GetFileNameWithoutExtend()))
                    .ToString();

                UIKitSettingData uiKitSettingData = UIKitSettingData.Load();

                string panelPrefabPath = $"{mModuleFolder}{uiKitSettingData.UIPrefabDir}/{panelName}"
                    .GetFolderPath()
                    .CreateDirIfNotExists()
                    .Builder()
                ...
                mCommands.Enqueue(() =>
                {
                    ...
                    GameObject uirootPrefab = Resources.Load<GameObject>("UIRoot");
                    ...
                    Transform designTransform = uiroot.transform.Find("Design");

                    GameObject gameObj = new(panelName.GetFileNameWithoutExtend())
                    {
                        transform =
                        {
                    ...
                    Object prefab = PrefabUtils.SaveAndConnect(panelPrefabPath, gameObj);
                    ...
                    GameObject tester = new("Test{0}".FillFormat(panelName));
                    ResKitUIPanelTester uiPanelTester = tester.AddComponent<ResKitUIPanelTester>();
        ...
        {
            public static string CreateUIPanel { get => LocaleKitEditor.IsCN.Value ? "创建 UI Panel" : "Create UI Panel"; }

            public static string ModuleFolder { get => LocaleKitEditor.IsCN.Value ? "模块目录:" : "Module Folder"; }

            public static string ResolutionOrMatchWidthOrHeight { get => LocaleKitEditor.IsCN.Value ? "分辨率&对齐:" : "Resolution&MatchWidthOrHeight:"; }
            ...
            public static string Preview { get => LocaleKitEditor.IsCN.Value ? "生成文件预览:" : "Preview:"; }
