...
namespace QFramework
{
    public class Guideline
    {
        ...
        public class GuidelineItemFolder
        {
            ...
            {
                foreach (GuidelineItemFolder guidelineItemGroup in Folders.OrderBy(g => g.Name))
                {
                    if (guidelineItemGroup is GuidelineItem)
                    {
                        GuidelineItem guidelineItem = guidelineItemGroup as GuidelineItem;
                            ...
                            Event.current.type == EventType.MouseUp)
                        {
                            ...
                            TextAsset textAsset = AssetDatabase.LoadAssetAtPath<TextAsset>(guideline.mSelectedView.FilePath);
        ...
        public void OnShow()
        {
            ...
            TextAsset positionMarkForLoad = Resources.Load<TextAsset>("EditorGuideline/PositionMarkForLoad");

            string path = AssetDatabase.GetAssetPath(positionMarkForLoad);
            string folderPath = path.GetFolderPath();
            string folderName = folderPath.GetFileName();
            string[] markdownFilePaths = Directory.GetFiles(folderPath, "*.md", SearchOption.AllDirectories);

            mMarkdownViewer = new MDViewer(Resources.Load<GUISkin>("Skin/MarkdownSkinQS"), path, "");

            foreach (string filePath in markdownFilePaths)
            {
            ...
            foreach (GuidelineItem guidelineItem in mViews)
            {
                string pathToRoot = guidelineItem.FilePath.RemoveString(folderPath);

                string[] names = pathToRoot.Split(Path.DirectorySeparatorChar);

                GuidelineItemFolder currentGroup = mRootFolder;
                int indent = 1;
                foreach (string name in names)
                {
                    ...
                    {
                        GuidelineItemFolder childGroup = currentGroup.Folders.FirstOrDefault(g => g.Name == name);
            ...
            mLeftLayout = EasyIMGUI.Area().WithRectGetter(() => mLeftRect)
                ...
                .AddChild(EasyIMGUI.Custom().OnGUI(() =>
                {
                    ...
                    if (GUILayout.Button(LocaleKitEditor.IsCN.Value ? "导出" : "Export"))
                    {
                        ...
                        {
                            foreach (GuidelineItemFolder guidelineItemGroup in currentFolder.Folders)
                            {
                                if (guidelineItemGroup is GuidelineItem)
                                {
                                    GuidelineItem guidelineItem = guidelineItemGroup as GuidelineItem;
                                    string content = File.ReadAllText(guidelineItem.FilePath);
                        ...
                        PackageVersion framework = PackageKit.Interface.GetModel<ILocalPackageVersionModel>()
                            .GetByName("Framework");


                        string guidelineText = LocaleKitEditor.IsCN.Value ? "使用指南 " : "Guideline";

                        string savedPath = EditorUtility.SaveFilePanel($"QFramework {framework.Version} {guidelineText}",
