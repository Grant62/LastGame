...
#if UNITY_5_6_OR_NEWER
        private void LoadResourcesUnityNative(string assetsPath)
        {
            try
            {
                Object[] array = Resources.LoadAll(assetsPath, typeof(TextAsset));

                for (int i = 0; i < array.Length; i++)
                {
                    TextAsset o = (TextAsset)array[i];
            ...
            {
                Debug.LogErrorFormat("Error initializing UnityScriptLoader : {0}", ex);
            }
        }
