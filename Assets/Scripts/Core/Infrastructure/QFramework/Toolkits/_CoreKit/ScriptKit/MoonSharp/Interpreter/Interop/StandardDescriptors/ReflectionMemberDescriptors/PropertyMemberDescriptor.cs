            ...
            using (PerformanceStatistics.StartGlobalStopwatch(PerformanceCounter.AdaptersCompilation))
            {
                if (m_Getter != null)
                {
                    if (IsStatic)
                    {
                        ...
                        Expression<Func<object, object>> lambda = Expression.Lambda<Func<object, object>>(castPropAccess, paramExp);
                    ...
                    else
                    {
                        ...
                        UnaryExpression castParamExp = Expression.Convert(paramExp, PropertyInfo.DeclaringType);
                        MemberExpression propAccess = Expression.Property(castParamExp, PropertyInfo);
                        UnaryExpression castPropAccess = Expression.Convert(propAccess, typeof(object));
                        Expression<Func<object, object>> lambda = Expression.Lambda<Func<object, object>>(castPropAccess, paramExp);
                        Interlocked.Exchange(ref m_OptimizedGetter, lambda.Compile());
                    }
        ...
        internal void OptimizeSetter()
        {
            using (PerformanceStatistics.StartGlobalStopwatch(PerformanceCounter.AdaptersCompilation))
            {
                if (m_Setter != null && !Framework.Do.IsValueType(PropertyInfo.DeclaringType))
                {
                    ...
                    {
                        ParameterExpression paramExp = Expression.Parameter(typeof(object), "dummy");
                        ParameterExpression paramValExp = Expression.Parameter(typeof(object), "val");
                        UnaryExpression castParamValExp = Expression.Convert(paramValExp, PropertyInfo.PropertyType);
                        MethodCallExpression callExpression = Expression.Call(setterMethod, castParamValExp);
                        Expression<Action<object, object>> lambda = Expression.Lambda<Action<object, object>>(callExpression, paramExp, paramValExp);
                    ...
                    else
                    {
                        ...
                        UnaryExpression castParamExp = Expression.Convert(paramExp, PropertyInfo.DeclaringType);
                        UnaryExpression castParamValExp = Expression.Convert(paramValExp, PropertyInfo.PropertyType);
                        MethodCallExpression callExpression = Expression.Call(castParamExp, setterMethod, castParamValExp);
                        Expression<Action<object, object>> lambda = Expression.Lambda<Action<object, object>>(callExpression, paramExp, paramValExp);
                        Interlocked.Exchange(ref m_OptimizedSetter, lambda.Compile());
                    }
                }
