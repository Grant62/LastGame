...
namespace MoonSharp.Interpreter
{
    ...
    public static class ModuleRegister
    {
        ...
        public static Table RegisterModuleType(this Table gtable, Type t)
        {
            ...
            foreach (FieldInfo fi in Framework.Do.GetFields(t).Where(_mi => _mi.IsStatic && _mi.GetCustomAttributes(typeof(MoonSharpModuleMethodAttribute), false).Length > 0))
            {
            ...
            foreach (FieldInfo fi in Framework.Do.GetFields(t).Where(_mi => _mi.IsStatic && _mi.GetCustomAttributes(typeof(MoonSharpModuleConstantAttribute), false).Length > 0))
            {
