...
namespace MoonSharp.Interpreter.Interop.BasicDescriptors
{
    ...
    public abstract class DispatchingUserDataDescriptor : IUserDataDescriptor, IOptimizableDescriptor
    {
            ...
            if (odesc != null)
            {
                if (members.ContainsKey(name))
                {
                    ...
                    if (overloads != null)
                        ...
                        throw new ArgumentException(string.Format("Multiple members named {0} are being added to type {1} and one or more of these members do not support overloads.", name,
                            Type.FullName));
                }
                else
                {
        ...
        {
            foreach (IOptimizableDescriptor m in m_MetaMembers.Values.OfType<IOptimizableDescriptor>())
                m.Optimize();

            foreach (IOptimizableDescriptor m in m_Members.Values.OfType<IOptimizableDescriptor>())
                m.Optimize();
        }
        ...
        {
            return DynValue.NewCallback((context, args) => DynValue.NewBoolean(CheckEquality(obj, args[0].ToObject(), args[1].ToObject())));
        }
