...
namespace QFramework
{
    ...
    internal class Custom<TData> : IAction, ICustomAPI<TData>
    {
        ...
        private static readonly SimpleObjectPool<Custom<TData>> mSimpleObjectPool = new(() => new Custom<TData>(), null, 10);

        protected Custom() { }

        public static Custom<TData> Allocate()
        {
            Custom<TData> custom = mSimpleObjectPool.Allocate();
    ...
    internal class Custom : Custom<object>
    {
        ...
        {
            Custom custom = mSimpleObjectPool.Allocate();
            custom.Deinited = false;
            custom.Reset();
            return custom;
        }
    ...
    public static class CustomExtension
    {
        ...
        {
            IAction custom = ActionKit.Custom(onCustomSetting);
