...
namespace QFramework
{
    ...
    public partial class GUIGraphWindow
    {
        ...
        public static void BeginZoomed(Rect rect, float zoom, float topPadding)
        {
            ...
            Vector4 padding = new(0, topPadding, 0, 0);
            padding *= zoom;
            ...
            foreach (GUIGraphNode node in graph.nodes)
            {
                ...
                foreach (GUIGraphNodePort output in node.Outputs)
                {
                    ...
                    for (int k = 0; k < output.ConnectionCount; k++)
                    {
                        ...
                        {
                            GUIGraphRerouteReference rerouteRef = new(output, k, i);
        ...
        private void DrawNodes()
        {
            ...
            List<Object> preSelection = preBoxSelection != null
            ...
            Rect selectionBox = new(boxStartPos, boxSize);

            //Save guiColor so we can revert it
            Color guiColor = GUI.color;

            List<GUIGraphNodePort> removeEntries = new();
            ...
            for (int n = 0; n < graph.nodes.Count; n++)
            {
                ...
                {
                    removeEntries.Clear();
                    foreach (KeyValuePair<GUIGraphNodePort, Rect> kvp in portConnectionPoints)
                        if (kvp.Key.node == node)
                            removeEntries.Add(kvp.Key);
                    foreach (GUIGraphNodePort k in removeEntries) portConnectionPoints.Remove(k);
                }
                ...
                if (selected)
                {
                    GUIStyle style = new(nodeEditor.GetBodyStyle());
                    GUIStyle highlightStyle = new(nodeEditor.GetBodyHighlightStyle());
                ...
                if (e.type == EventType.Repaint)
                {
                    ...
                    foreach (KeyValuePair<GUIGraphNodePort, Vector2> kvp in GUIGraphNodeEditor.portPositions)
                    {
                        Vector2 portHandlePos = kvp.Value;
                        portHandlePos += node.position;
                        Rect rect = new(portHandlePos.x - 8, portHandlePos.y - 8, 16, 16);
                ...
                if (e.type != EventType.Layout)
                {
                    ...
                    Rect windowRect = new(nodePos, nodeSize);
        ...
        private void DrawTooltip()
        {
                ...
                graphEditor != null)
            {
                ...
                size.x += 8;
                Rect rect = new(Event.current.mousePosition - size, size);
