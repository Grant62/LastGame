...
namespace QFramework
{
    ...
    ///     renaming a <see cref="XNode.NodeGraph" /> asset, it appears that sometimes the v2 AssetDatabase will swap which
    ///     asset
    ///     is the main asset (present at top level) between the <see cref="XNode.NodeGraph" /> and one of its
    ///     <see cref="XNode.Node" />
    ///     sub-assets. As a workaround until Unity fixes this, this asset processor checks all renamed assets and if it
    ///     finds a case where a <see cref="XNode.Node" /> has been made the main asset it will swap it back to being a
    ///     sub-asset
