...
namespace QFramework
{
    ...
    public static class GUIGraphDataCache
    {
        ...
        private static bool Initialized { get => portDataCache != null; }
        ...
        public static void UpdatePorts(GUIGraphNode node, Dictionary<string, GUIGraphNodePort> ports)
        {
            ...
            Dictionary<string, GUIGraphNodePort> staticPorts = new();
            Dictionary<string, List<GUIGraphNodePort>> removedPorts = new();
            Type nodeType = node.GetType();

            List<GUIGraphNodePort> dynamicListPorts = new();
            ...
            foreach (GUIGraphNodePort staticPort in staticPorts.Values)
            {
                ...
                {
                    GUIGraphNodePort port = new(staticPort, node);
        ...
        private static void BuildCache()
        {
            ...
            Type baseType = typeof(GUIGraphNode);
        ...
        [Serializable]
        private class PortDataCache : Dictionary<Type, List<GUIGraphNodePort>>, ISerializationCallbackReceiver
        {
            private List<Type> keys = new();
            private List<List<GUIGraphNodePort>> values = new();
            ...
            public void OnBeforeSerialize()
            {
                keys.Clear();
                values.Clear();
                foreach (KeyValuePair<Type, List<GUIGraphNodePort>> pair in this)
                {
                    keys.Add(pair.Key);
