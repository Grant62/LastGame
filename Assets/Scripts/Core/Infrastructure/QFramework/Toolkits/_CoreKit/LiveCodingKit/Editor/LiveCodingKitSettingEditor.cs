...
namespace QFramework
{
    ...
    internal class LiveCodingKitSettingEditor : IPackageKitView
    {
        ...
        private class LocaleText
        {
            ...
            public static string Switch { get => IsCN ? " 开启：" : "Open:"; }


            public static string Apply { get => IsCN ? "保存" : "Apply"; }

            public static string WhenCompileFinish { get => IsCN ? "当编译完成时" : "When Compile Finish"; }

            private static string ReloadCurrentScene { get => IsCN ? "重新加载当前场景" : ""; }

            private static string RestartGame { get => IsCN ? "重启游戏" : ""; }
