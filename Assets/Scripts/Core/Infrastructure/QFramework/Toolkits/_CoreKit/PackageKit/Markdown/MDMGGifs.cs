...
namespace QFramework
{
    ...
    public class Decoder : IDisposable
    {
        ...
        private void ReadHeader()
        {
            ...
            ImageFlag flags = (ImageFlag)ReadByte();
            byte bgIndex = ReadByte(); // background colour
        ...
        public MDGifImage NextImage()
        {
            ...
            while (true)
            {
                ...
                switch (block)
                {
                    ...
                    {
                        Extension ext = (Extension)ReadByte();
        ...
        {
            int tableSize = Pow2[(int)(flags & ImageFlag.TableSizeMask) + 1];
        ...
        private void ReadControlBlock()
        {
            ...
            ControlFlags flags = (ControlFlags)ReadByte(); // flags
            Image.Delay = ReadUInt16() * 10; // delay (1/100th -> milliseconds)
            byte transparentColour = ReadByte(); // transparent colour
        ...
        {
            int numRows = Output.Length / Width;
            int writePos = Output.Length - Width; // NB: work backwards due to Y-coord flip
            Color32[] input = Output;
 ...
 Marshal.ReAllocHGlobal( CodesHandle, (IntPtr)( CodesLength * sizeof( ushort ) ) );
        ...
        private void DecompressLZW()
        {
            ...
            int nextSize = Pow2[codeSize];
            int maximumCodeSize = Pow2[minimumCodeSize];
            int clearCode = maximumCodeSize;
            int endCode = maximumCodeSize + 1;

            // initialise buffers

            int codesEnd = 0;
            int numCodes = maximumCodeSize + 2;
            ...
            while (true)
            {
                ...
                else
                {
                    ...
                    if (bitsAvailable > 0)
                    {
                        int bitsRemaining = codeSize - bitsAvailable;
                        curCode |= shiftRegister << bitsAvailable & mask;
                        shiftRegister >>= bitsRemaining;
                        bitsAvailable = newBits - bitsRemaining;
                    }
                ...
                ushort codeLength = Codes[codePos++];
                ushort newCode = Codes[codePos];

                for (int i = 0; i < codeLength; i++)
                {
                    ushort code = Codes[codePos++];
        ...
        {
            string v = "1.1";
            string e = BitConverter.IsLittleEndian ? "L" : "B";
