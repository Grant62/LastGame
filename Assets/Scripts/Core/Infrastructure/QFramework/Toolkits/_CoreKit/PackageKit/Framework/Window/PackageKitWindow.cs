...
namespace QFramework
{
    ...
    public class PackageKitWindow : EasyEditorWindow
    {
        ...
        {
            IBindableProperty<bool> _ = LocaleKitEditor.IsCN;
        }
        ...
        private static void InitializeViews(List<IPackageKitView> views)
        {
            ...
            Type[] viewTypes = PackageKitAssemblyCache.GetDerivedTypes<IPackageKitView>(false, false).ToArray();

            foreach (Type view in viewTypes)
            {
                if (view.GetAttribute<PackageKitIgnoreAttribute>() == null)
                {
                    IPackageKitView viewInstance = Activator.CreateInstance(view) as IPackageKitView;
                    if (viewInstance == null) continue;
                    views.Add(viewInstance);
                }
            }

            foreach (IPackageKitView view in views)
            {
        ...
        {
            public static string QFrameworkSettings { get => LocaleKitEditor.IsCN.Value ? "QFramework 设置" : "QFramework Settings"; }

            public static string Compiling { get => LocaleKitEditor.IsCN.Value ? "编译中..." : "Compiling..."; }

            public static string Reloading { get => LocaleKitEditor.IsCN.Value ? "重载中..." : "Reloading..."; }
        }
        ...
        {
            PackageKitWindow packageKitWindow = Create<PackageKitWindow>(true);
        ...
        protected override void Init()
        {
            ...
            IEnumerable<PackageKitViewRenderInfo> reflectRenderInfos = AssetDatabase.FindAssets("packagekitconfig t:TextAsset")
                ...
                .Select(path =>
                {
                    ...
                    PackageKitScriptViewRenderInfo infoNew = JsonUtility.FromJson<PackageKitScriptViewRenderInfo>(jsonText);

                    if (infoNew.Load())
                    {
                        PackageKitViewRenderInfo renderInfo = new(infoNew)
                        {
        ...
        private PackageKitViewRenderInfo mSelectedViewRender;

        private void LeftSelectView(string search)
        {
            foreach (KeyValuePair<MutableTuple<string, bool>, List<PackageKitViewRenderInfo>> packageKitViewRenderInfo in mPackageKitViewRenderInfos.OrderBy(kv => kv.Key.Item1))
            {
                        ...
                        true))
                {
                    ...
                    foreach (PackageKitViewRenderInfo drawer in packageKitViewRenderInfo.Value)
                    {
                        GUILayout.BeginVertical();
