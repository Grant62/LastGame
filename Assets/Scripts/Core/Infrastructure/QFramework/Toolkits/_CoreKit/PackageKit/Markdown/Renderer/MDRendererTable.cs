...
namespace QFramework
{
    internal class MDRendererTable : MarkdownObjectRenderer<MDRendererMarkdown, Table>
    {
        protected override void Write(MDRendererMarkdown renderer, Table table)
        {
            ...
            int numCols = (table[0] as TableRow).Count(c => (c as TableCell).Count > 0);

            // column alignment
            TableColumnAlign[] alignment = table.ColumnDefinitions
            ...
            foreach (TableRow row in table)
            {
                ...
                bool consumeSpace = renderer.ConsumeSpace;
                renderer.ConsumeSpace = true;

                int numCells = Mathf.Min(numCols, row.Count);

                for (int cellIndex = 0; cellIndex < numCells; cellIndex++)
                {
                    TableCell cell = row[cellIndex] as TableCell;
                    ...
                    {
                        bool consumeNewLine = renderer.ConsumeNewLine;
