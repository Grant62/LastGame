...
namespace QFramework
{
    ...
    internal class PackageKitScriptViewRenderInfo : IPackageKitView
    {
        ...
        private readonly object[] mEmptyParams = { };
        #endregion
        ...
        public void OnShow()
        {
            if (Language == "lua")
            {
                string scriptFilePath = AssetDatabase.FindAssets($"{LuaFileName} t:TextAsset")
                    .Select(AssetDatabase.GUIDToAssetPath).FirstOrDefault(p => p.EndsWith(".lua"));

                if (scriptFilePath == null) return;
                TextAsset scriptText = AssetDatabase.LoadAssetAtPath<TextAsset>(scriptFilePath);
