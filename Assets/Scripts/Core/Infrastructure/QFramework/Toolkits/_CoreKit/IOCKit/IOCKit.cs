...
namespace QFramework
{
    ...
    public class QFrameworkContainer : IQFrameworkContainer
    {
        ...
        {
            foreach (object obj in ResolveAll(typeof(TType)))
            {
            ...
            params object[] constructorArgs)
        {
            ...
            {
                object obj = CreateInstance(namedMapping, constructorArgs);
                //Inject(obj);
                return obj;
            }
        ...
        public object CreateInstance(Type type, params object[] constructorArgs)
        {
            if (constructorArgs != null && constructorArgs.Length > 0)
            {
                ...
                object obj2 = Activator.CreateInstance(type, constructorArgs);
                Inject(obj2);
                return obj2;
            }
#if !NETFX_CORE
        ...
        ConstructorInfo[] constructor = type.GetTypeInfo().DeclaredConstructors.ToArray();
            ...
            object[] args = maxParameters.Select(p =>
            {
        ...
        {
            Type concreteType = RelationshipMappings[tfor, tbase];
            ...
            object result = CreateInstance(concreteType, args);
            //Inject(result);
            return result;
        }
    ...
    public class TypeMappingCollection : Dictionary<Tuple<Type, string>, Type>
    {
        public Type this[Type from, string name = null]
        {
            get
            {
                Tuple<Type, string> key = new(from, name);
                Type mapping = null;
                if (TryGetValue(key, out mapping))
                {
                    return mapping;
                }
    ...
    public class TypeRelationCollection : Dictionary<Tuple<Type, Type>, Type>
    {
        public Type this[Type from, Type to]
        {
            get
            {
                Tuple<Type, Type> key = new(from, to);
                Type mapping = null;
                if (TryGetValue(key, out mapping))
                {
                    return mapping;
